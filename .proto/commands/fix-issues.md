...
2. Run `gh search prs` for issue number. Reasonable existing PR → review per `.proto/commands/review-prs.md`, report `existing-pr`; do NOT create competing fix.
...
WT="$HOME/.proto/wt/${ENC}/fix-issue-<N>"
...
Worktree:  ~/.proto/wt/.../fix-issue-<N>            (if created)
