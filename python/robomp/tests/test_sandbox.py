...
def _workspace(root: Path) -> Workspace:
    return Workspace(
        root=root,
        repo_dir=root / "repo",
        session_dir=root / ".proto-session",
...
def test_release_workspace_resets_to_remote_main_and_uses_tag_session(
    ...
    assert workspace.session_dir.name == ".proto-session-v1.2.3"
    ...
    assert next_release.session_dir.name == ".proto-session-v1.2.4"
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert tmpdir == tmp_path / ".proto-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".proto-xdg" / "data", tmp_path / ".proto-xdg" / "state", tmp_path / ".proto-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "proto").is_dir()
    assert (tmp_path / ".proto-xdg" / "cache" / "bun-install").is_dir()
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    bun_cache = ws.root / ".proto-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".proto-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".proto-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".proto-xdg" / "data", ws.root / ".proto-xdg" / "state", ws.root / ".proto-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "proto").is_dir()
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".proto-tmp",
            ws_root / ".proto-xdg" / "data",
            ws_root / ".proto-xdg" / "data" / "proto",
            ws_root / ".proto-xdg" / "state",
            ws_root / ".proto-xdg" / "state" / "proto",
            ws_root / ".proto-xdg" / "cache",
            ws_root / ".proto-xdg" / "cache" / "proto",
            ws_root / ".proto-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".proto-tmp",
        ws.root / ".proto-xdg" / "data",
        ws.root / ".proto-xdg" / "data" / "proto",
        ws.root / ".proto-xdg" / "state",
        ws.root / ".proto-xdg" / "state" / "proto",
        ws.root / ".proto-xdg" / "cache",
        ws.root / ".proto-xdg" / "cache" / "proto",
        ws.root / ".proto-xdg" / "cache" / "bun-install",
    }
...
def _seed_reclaimable_workspace(mgr: SandboxManager, repo: str, number: int) -> Path:
    ...
    for rel in (
        ...
        "repo/src",
        ".proto-session",
        ".proto-xdg/cache/bun-install",
        ".proto-xdg/state/proto",
        ".proto-tmp",
        "artifacts",
    ):
    ...
    (ws_root / ".proto-session/session.jsonl").write_text("{}", encoding="utf-8")
    (ws_root / ".proto-xdg/cache/bun-install/pkg.tgz").write_text("x", encoding="utf-8")
    (ws_root / ".proto-xdg/state/proto/state.json").write_text("{}", encoding="utf-8")
    (ws_root / ".proto-tmp/scratch").write_text("x", encoding="utf-8")
...
def test_reclaim_workspace_caches_strips_dep_caches_and_preserves_state(tmp_path: Path) -> None:
    ...
    assert not (ws_root / ".proto-xdg/cache").exists()
    assert not (ws_root / ".proto-tmp").exists()
    assert (ws_root / "repo/src/keep.ts").read_text(encoding="utf-8") == "keep"
    assert (ws_root / ".proto-session/session.jsonl").exists()
    assert (ws_root / ".proto-xdg/state/proto/state.json").exists()
