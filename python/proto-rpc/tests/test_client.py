...
from proto_rpc.client import _RpcFrameDecoder
...
class RpcClientTests(unittest.TestCase):
    ...
    def test_xd_dispatched_custom_tool_events_carry_host_tool_name(self) -> None:
        ...
        With `tools.xdev` on, proto invokes a custom tool through `write
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the proto process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. proto is now spawned in its own
