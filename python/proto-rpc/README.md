...
Typed Python bindings for the `proto --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `proto --mode rpc` flags such as thinking level,
...
```python
from proto_rpc import MessageUpdateEvent, RpcClient
...
from proto_rpc import RpcClient, host_tool
...
```python
from proto_rpc import RpcClient, host_uri
...
```python
from proto_rpc import assistant_text, assistant_text_with_thinking
