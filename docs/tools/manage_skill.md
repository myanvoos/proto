...
- Tool metadata: `loadMode = "discoverable"`. Like other discoverable tools it mounts under `xd://` for on-demand discovery instead of staying in the always-on essential set.
...
## Side Effects
- Filesystem: writes or deletes `<agent-dir>/managed-skills/<name>/SKILL.md`; the default agent directory is `~/.proto/agent`.
- Network: none.
