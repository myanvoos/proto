...
PROTO-owned dialect and therefore is not a valid `tools.format` value.

The proto renderer always writes a nested `arguments` object and renders
...
- **Robustness:** the format is prompt/template-driven, so malformed output is possible
  ...
  arguments). vLLM may fall back to content depending on its parser path; proto's
  ...
  on that backend.
- **Version/scope:** this `hermes` template covers `Qwen3-*`, `Qwen2.5-*`, and `QwQ-32B`. It does **not** cover `Qwen3-Coder`, which uses a different XML scheme parsed by a serving engine's `qwen3_xml` parser. PROTO has no `qwen3_xml` owned dialect; use `tools.format=native` and configure that parser at the endpoint.
