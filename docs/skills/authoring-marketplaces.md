...
description: Use when creating a new proto marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---

# Authoring Marketplaces

A marketplace is a Git repository (or local directory) that contains a catalog file at either `.proto-plugin/marketplace.json` (preferred for proto-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
The catalog file lives at either `.proto-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. proto prefers the `.proto-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "plugins": [
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/proto-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
my-plugin/
  ...
  package.json                   ← optional version and `proto.extensions`
  README.md                      ← recommended: description + usage
```

> Note: MCP servers may instead be declared by the manifest's `mcpServers` field — either an inline server map or a path to a config file inside the plugin root (`{ "mcpServers": "./mcp-proto.json" }`). proto reads `.proto-plugin/plugin.json` first, then `.claude-plugin/plugin.json`; a manifest declaration replaces the default `.mcp.json` rather than merging with it, so one published tree can carry a per-harness MCP config.

> Note: extension modules declared via `package.json` `proto.extensions` **are** loaded from marketplace installs — installation symlinks the cached plugin into the scope's `node_modules` and records it in `proto-plugins.lock.json`, the same runtime surfaces used by npm-installed and `proto plugin link`ed plugins.
...
```
proto plugin marketplace add owner/repo
proto plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in the user plugins data root's `installed_plugins.json` (`~/.proto/plugins/installed_plugins.json` by default), available in all projects. On Linux and macOS, `proto config init-xdg` creates (but does not migrate data into) the XDG roots; once the relevant roots exist and the XDG variables are set, new user state uses `$XDG_DATA_HOME/proto/plugins/installed_plugins.json`.
- **project** — installed in `<project>/.proto/plugins/installed_plugins.json`, available only in that project
...
1. Create `marketplace.json` at `.proto-plugin/marketplace.json` (proto-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
