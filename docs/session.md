...
```text
~/.proto/agent/sessions/<encoded-cwd>/<timestamp>_<sessionId>.jsonl
```
...
| `orchestrator-worker-lifecycle` | Version-1 event with `{ version: 1, id, ownerId, parentSessionId, action, ... }`; spawns add agent/session metadata and optional effort/schema controls; turn events add `turn`; tombstones add `reason`. | Replays parent-owned persistent workers and durable terminal state; invalid or out-of-scope events are ignored. |
