...
It does **not** cover TypeScript/JavaScript extension module loading (`extensions/*.ts`, `index.ts`, `package.json proto.extensions`), which is documented in [Extension Loading](./extension-loading.md).
...
`<name>/index.ts` / `index.js`, and `package.json` `proto`/`pi` extension entries.
