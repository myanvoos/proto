...
ACP (`proto acp`) uses the same settings resolver as normal PROTO launches. Global `~/.proto/agent/config.yml` applies, project config for the ACP session `cwd` applies, and any `--config <file>` overlays passed to the ACP server process apply to sessions created by that process.
