...
└───────────────────────────────────────┘
 ...
 (native provider scans project .proto before user .proto;
...
1. Global settings: the first present file among `~/.proto/agent/config.yml` and `config.yaml`
2. Project settings: discovered via the settings capability (`settings.json` and `config.yml` from providers)
3. Config overlays: `PI_CONFIG_FILES` (platform path-list), followed by repeated `proto --config <path>` files; all are loaded as `config.yml`-style YAML for this process only
...
## 6) Native `.proto` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.proto/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.proto/agent/skills`, without requiring the root `.proto` directory itself to be non-empty.
- `SYSTEM.md`, `RULES.md`, and `.proto/AGENTS.md` read user-level files directly and use the nearest non-empty ancestor `.proto` directory for project files. `RULES.md` becomes an always-apply sticky rule. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract.
- MCP does not use the non-empty-root admission helper. It reads project `.proto/mcp.json` then `.proto/.mcp.json`, followed by user `mcp.json` then `.mcp.json`, directly.

### Scope-specific loading

- Skills: `<ancestor>/.proto/skills/*/SKILL.md` and `~/.proto/agent/skills/*/SKILL.md`
...
- Context files: `.proto/AGENTS.md`; standalone ancestor `AGENTS.md` files are loaded separately by the low-priority `agents-md` provider

### Nearest-project lookup nuance

For `SYSTEM.md`, `RULES.md`, and `.proto/AGENTS.md`, the native provider walks upward to the nearest non-empty project `.proto` directory.
...
- Field migrations cover renamed/removed settings and value-shape changes, including `queueMode`, changelog settings, `ask.timeout`, flat `theme`, `inspect_image.enabled`, worker isolation/eager settings, removed edit and compaction modes, `inlineToolDescriptors`, status-line segments, provider/search settings, memories/hindsight settings, and nested-leaf renames. Consult `Settings.#migrateRawSettings()` for the current exhaustive list.
