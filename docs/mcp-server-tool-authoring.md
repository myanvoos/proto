...
block is optional. PROTO honors an explicit arbitrary or legacy credential ID when
...
block—PROTO looks for a credential under deterministic IDs derived from the
...
If no credential resolves, PROTO connects without injecting an OAuth value.
Refresh or credential-resolution failures are logged; when possible, PROTO
