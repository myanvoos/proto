...
- **user** (default) -- available in all projects, stored in the user plugins data root's `installed_plugins.json` (`~/.proto/plugins/installed_plugins.json` by default)
- **project** -- available only in the active project, stored in the nearest project `.proto/plugins/installed_plugins.json`
...
On Linux and macOS, `proto config init-xdg` creates the XDG data, state, and cache roots; it does not move existing data. Once the relevant roots exist and `XDG_DATA_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` are set, new user marketplace/plugin state resolves under `$XDG_DATA_HOME/proto` (including `marketplaces.json` and `plugins/`). The `~/.proto` paths below are the non-XDG defaults.
...
```
proto plugin marketplace add <source>
proto plugin marketplace remove <name>
proto plugin marketplace update [name]
proto plugin marketplace list
proto plugin discover [marketplace]
proto plugin install [--force] [--scope user|project] name@marketplace
proto plugin uninstall [--scope user|project] name@marketplace
proto plugin upgrade [--scope user|project] [name@marketplace]
proto plugin enable [--scope user|project] name@marketplace
proto plugin disable [--scope user|project] name@marketplace
proto plugin list
...
Git and local sources must contain a catalog at `.proto-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.proto-plugin/marketplace.json` in the repository root. When proto is the only intended consumer, prefer this path. To remain Claude Code-compatible (proto loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — proto uses it as a fallback when `.proto-plugin/marketplace.json` is absent. A repository may ship both: proto reads the `.proto-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
~/.proto/
  ...
  plugins/
    ...
    proto-plugins.lock.json         # Runtime enable/feature state
...
<project>/.proto/
  plugins/
    ...
    proto-plugins.lock.json         # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
