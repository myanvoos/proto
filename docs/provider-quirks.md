...
- **Usage Accounting & Quota Surfacing**: `calculateOpenAIUsageAccounting` (`packages/ai/src/providers/openai-shared.ts`) reconciles input, output, cache-read, and cache-write tokens into standard `Usage` records. OpenRouter authoritative charges are populated into `output.usage.cost` via `applyProviderReportedCost`. Copilot request counts are stored in `output.usage.premiumRequests`. Transport HTTP errors (e.g. 429 Rate Limit, 408 Timeout, 5xx Server Error) are thrown as `OpenAIHttpError` (`packages/ai/src/utils/openai-http.ts`), capturing status, headers, and error envelope details for upstream error mapping in `AIError.finalize`.
...
- **Responses Lite transport**: Normal inference uses full Responses by default; callers opt into Lite with the `responsesLite` request option or `PI_CODEX_RESPONSES_LITE=1` (`resolveCodexResponsesLite`), while provider-native compaction explicitly follows the catalog's `useResponsesLite` flag. Function `applyCodexResponsesLiteShape` embeds declared tools into a leading `additional_tools` developer item, system instructions into a developer message, strips image `detail`, turns off parallel tool calls, forces `reasoning.context: "all_turns"`, and appends `x-openai-internal-codex-responses-lite: true` header (or `ws_request_header_x_openai_internal_codex_responses_lite` in WS `client_metadata`). Hosted tool choices (`tool_choice`) fall back to `"auto"` if no matching declared tool is present (#5771).
...
- **Dynamic discovery**: `fetchCodexModels` in `packages/catalog/src/discovery/codex.ts` reads the provider-keyed Pi catalog at `https://pi.dev/api/models/providers/openai-codex` and maps its model metadata into `ModelSpec<"openai-codex-responses">`.
...
- **Strict responses tool-pairing**: Enabled by default for Azure OpenAI models via `buildOpenAIResponsesCompat` (`packages/catalog/src/compat/openai.ts`, `isAzure = true`). In `buildResponsesInput` / `appendResponsesToolResultMessages` (`packages/ai/src/providers/openai-shared.ts`), unpaired tool outputs (results whose `callId` was not emitted by a prior assistant `function_call` item) are rejected by Azure's strict backend. Proto folds orphan tool results into synthetic assistant note messages (`[Orphan <tool> result; call_id=<id>]: <text>` up to 16,000 characters, or `[Orphan computer result; call_id=<id>]`) rather than sending un-paired output items.
...
- **Usage & Quota Tracker**: `kimiUsageProvider` in `packages/ai/src/usage/kimi.ts` targets `GET /coding/v1/usages` (`https://api.kimi.com/coding/v1/usages`, configurable via `KIMI_CODE_BASE_URL`) with OAuth bearer token and `getKimiCommonHeaders()`.
  - Short-circuits when credentials are expired (`credential.expiresAt <= nowMs`). Parses `KimiUsagePayload`: maps `usage` object to a `Weekly limit` summary row (canonical `7d` window), purchased `totalQuota` to a `Total quota` row on its own window, `usages` monthly aggregates (`limit_month_total`, `limit_month_code`; `used_ratio` → percent rows, `limit_5h` skipped as a duplicate of the burst limit), and `limits` array (extracting `detail` and `window` duration/timeUnit) into `UsageLimit` entries, resolving reset timestamps via `parseResetTime` (`reset_at`, `resetTime`, `ttl`).
...
- **Usage & Quota**: Quota tracking is registered via `ollamaUsageProvider` and `ollamaCloudUsageProvider` in `packages/ai/src/usage/ollama.ts`. Neither provider exposes a standalone usage/quota API (`validatesCredentials: false`, empty `limits`), relying on per-response `prompt_eval_count` (total prompt), `prompt_eval_cached_count` (Ollama Cloud's cached prompt tokens → `cacheRead`, subtracted from input; absent locally), and `eval_count` (output) returned in stream completion chunks.
...
- **`gitlab-duo-agent` Tool Execution Protocol**: Maps PROTO tools into MCP tool definitions (`buildGitLabDuoWorkflowMcpTools`, `GitLabMcpToolDefinition`) sent in `startRequest.mcpTools`. Tool invocation requests (`runMCPTool`, `run_mcp_tool`) received over WebSocket are extracted (`extractGitLabDuoWorkflowAction`), dispatched to PROTO tool execution (`mapGitLabDuoWorkflowActionToOmpTool`, `emitGitLabDuoWorkflowActionToolCall`), and returned via `buildGitLabDuoWorkflowActionResponse`.
...
Charm's OpenAI-compatible inference gateway for coding agents (`https://hyper.charm.land/v1`). Transport: OpenAI Chat Completions.

### Special casings
- **Base URL Normalization**: `normalizeCharmHyperBaseUrl` in `packages/catalog/src/wire/charm-hyper.ts` is shared by discovery, the `/credits` usage probe, and the model-cache namespace. A blank override means "not configured"; any other value keeps its host and gains `/v1`.
- **Wire dialect**: per-model compat sets `supportsStore: false`, `supportsDeveloperRole: false`, `maxTokensField: "max_tokens"`, `thinkingFormat: "openai"` (`reasoning_effort`), and `reasoningContentField: "reasoning_content"`. The gateway answers 200 to unknown fields and unadvertised efforts, so acceptance is not evidence of support.

### Auth & usage
- **API key**: `CHARM_HYPER_API_KEY`, then `HYPER_API_KEY`. `/login charm-hyper` strips a pasted `Bearer ` prefix and validates against `/v1/credits`, because the public `/v1/models` answers 200 for any key.
- **Credit balance**: `charmHyperUsageProvider` (`packages/ai/src/usage/charm-hyper.ts`) reports `{"balance": N}` as a remaining-only `credits` limit marked `scope.shared`: the balance is account-wide, so `/usage` collapses rows from several keys instead of summing them. 401/403 throws (purging the cached report); other failures are transient.

### Catalog model handling
- **Runtime-only, authoritative discovery**: the descriptor sets `allowUnauthenticated` and `dynamicModelsAuthoritative` but no `catalogDiscovery`, so the live roster is never frozen into the bundle. The cache namespace is endpoint-scoped (not credential-scoped) because the roster does not vary by key.
- **Thinking**: the ladder is exactly the advertised `reasoning.effort_levels` (catalog `model-thinking.ts` treats it as provider-authored); `none` is the off switch (`reasoningDisableMode: "none-effort"`), never a rung. Rows without an effort vocabulary stay non-reasoning even if they emit `reasoning_content` (glm-5, glm-5.1, kimi-k2-thinking, minimax-m2.7); streamed reasoning still displays.
- **Limit patches**: the gateway publishes `max_output_tokens` as a flat fraction of the context window. `glm-5.1` takes 20275 and `minimax-m2.7` 26214 (same-window peers' values); `minimax-m3` is restored from the leaked 512K tier to 1M context / 128K output.
...
Command Code's Provider API (`https://api.commandcode.ai/provider`). Mixed transport: `claude-*` ids use the Anthropic-compatible Messages endpoint at the `/provider` root; every other id uses OpenAI Chat Completions under `/provider/v1`.

### Special casings
- **Neutral discovery**: the public `/v1/models` rows carry only ids, names, and context lengths. Reasoning, input modalities, pricing, and limits come solely from the reviewed tables in `commandCodeModelManagerOptions` (effort registry of the Command Code CLI, official rate card); same-id rows from other hosts never lend capabilities, and the descriptor sets `skipCrossProviderReferenceFills` so generation does not either.
- **Effort dials**: only ids in the CLI's effort registry expose `reasoning_effort` (Claude ids use adaptive thinking); every other id sends no effort control.
- **Output cap**: the API publishes no per-model maximum, so rows default to 65,536 output tokens (the CLI default); `Qwen/Qwen3.8-27B` and `z-ai/glm-5.3-flash` carry their registry caps, and `gpt-5.3-codex` its 272K prompt window.
- **Wire dialect**: chat rows send `max_tokens`, no `store`, no developer role; Claude rows disable eager tool-input streaming and long cache retention.

### Auth & usage
- **API key**: `COMMAND_CODE_API_KEY`, then `COMMANDCODE_API_KEY`. `/login commandcode` stores the key without a probe: the models endpoint is public, and a completions probe would bill the key.
- **Pricing**: DeepSeek V4 rows use the off-peak rates; Qwen 3.7 Plus/Flash and Grok 4.6 carry long-context tiers. Free models stay at zero cost.
...
## Fire Pass (`firepass`)
Fire Pass is a Fireworks AI subscription tier providing dedicated high-throughput router access to GLM 5.2 Fast and Kimi K3 Fast. It uses the OpenAI Chat Completions transport (`https://api.fireworks.ai/inference/v1`) with Fireworks router endpoint translation.

### Special casings
- **Wire Model ID Translation (`wireModelIdMode: "firepass"`)**: `buildOpenAICompat` (`packages/catalog/src/compat/openai.ts`) assigns `wireModelIdMode: "firepass"` for `firepass` or Fireworks fast router models (`isFireworksFastRouter`). `applyWireModelIdTransform` (`packages/ai/src/providers/openai-shared.ts`) uses `toFirepassWireModelId` (`packages/catalog/src/fireworks-model-id.ts`) to convert friendly catalog IDs (e.g., `glm-5.2-fast`) into Fireworks router wire IDs (`accounts/fireworks/routers/glm-5p2-fast`) by replacing dots with `p`.
...
- **Thinking Effort**: Ladders come from the model family in `packages/catalog/src/model-thinking.ts`: GLM 5.2 Fast exposes `minimal`–`max` (with `minimal` sent as `none` via the Fireworks effort map), Kimi K3 Fast exposes `low`/`high`/`max`.
...
- **Validation**: Dedicated `fpk_...` keys only authorize the router endpoint and fail on `/v1/models`. `loginFirepass` uses `validation.kind: "chat-completions"` targeting `accounts/fireworks/routers/glm-5p2-fast` directly.

### Catalog model handling
- **Descriptor**: Registered in `packages/catalog/src/provider-models/descriptors.ts` (`id: "firepass"`, `defaultModel: "glm-5.2-fast"`, `envVars: ["FIREPASS_API_KEY"]`).
- **Manager Options**: `firepassModelManagerOptions` (`packages/catalog/src/provider-models/openai-compat.ts`) returns a static configuration without dynamic discovery. `FIREPASS_STATIC_MODELS` (same file) seeds the bundled roster with the published Fire Pass tariff and limits; the generator never restores retired Fire Pass rows from the previous snapshot.
...
The `gitlab-duo-agent` provider connects PROTO to the GitLab Duo Workflow Service (DWS) for agentic execution over a WebSocket action-bridge protocol. It rides the `GitLab Duo` transport section.
...
## Meta Model API (`meta`)
Meta Model API is Meta's commercial API platform hosting first-party Muse Spark models. It interacts with the model service via the OpenAI Responses transport targeting `https://api.meta.ai/v1`.

### Special casings
- **Output Token Clamp Bypass**: `buildOpenAIResponsesCompat` (`packages/catalog/src/compat/openai.ts`) sets `clampOutputToModelMax` for `meta` and `muse-code`, so `resolveOpenAIResponsesOutputClamp` (`packages/ai/src/providers/openai-shared.ts`) lets requests output up to `model.maxTokens` (131,072 tokens) instead of the default 64,000 token ceiling (`OPENAI_MAX_OUTPUT_TOKENS`).
- **No forced tool choice**: `api.meta.ai/v1` accepts only `tool_choice: "auto"` and rejects `none`, `required`, and named choices with 400, so the same builder sets `supportsToolChoice: false` for `meta` and `muse-code` and the field is omitted.
...
### Catalog model handling
- **Descriptor & Management**: Defined in `CATALOG_PROVIDERS` (`packages/catalog/src/provider-models/descriptors.ts`) with `defaultModel: "muse-spark-1.1"`. Uses `metaModelManagerOptions` (`packages/catalog/src/provider-models/openai-compat.ts`), which drops the `muse-image-`/`muse-voice-` media SKUs `/v1/models` lists and maps each bare id through the bundled row, then the seed, then a Muse Spark lineage spec.
- **Static Bundled Models**: `META_MUSE_STATIC_MODELS` seeds Muse Spark 1.1, 1.2, 1.3, and the `-contributor` SKUs (named `Muse Spark 1.x (C)`, discounted because prompts are used for training):
  - 1,048,576 token context window and 131,072 token max output limit.
  - Multimodal input support (`text`, `image`).
  - Effort thinking (`minimal`…`xhigh`); only Muse Spark 1.3 standard adds `max`.
  - Compatibility flags `supportsReasoningEffort: true` and `includeEncryptedReasoning: true`.
- **Unseeded revisions**: Meta lists bare ids without capability metadata, so a revision the seed does not know yet (e.g. `muse-spark-1.4[-contributor]`) inherits the lineage window, tier pricing, and 5-tier ladder with its own display name instead of surfacing as a text-only model.

## Muse Code (`muse-code`)
Meta's Muse Code subscription serves the same Muse Spark models over the Meta Model API wire contract, authenticated with a Model API key the subscription mints.

### Auth & usage
- **Device login**: `loginMuseCode` (`packages/ai/src/registry/oauth/muse-code.ts`) runs RFC 8628 against `https://auth.meta.com/oidc/device/{authorization,token}/` (form-encoded, `x-api-version: 1.0.0`), then POSTs the account token to `https://api.meta.ai/muse-code/key` with `{ onboard: true }` to mint the key. Inactive subscriptions fail with a 403-classified error; `require_payment`/`action_url` responses fail as `entitlement` with the purchase link.
- **Credential shape**: the OAuth `access` stores `{ oauthAccessToken, apiKey }` as JSON. Meta rejects `refresh_token` grants, so the provider registers no refresh hook and declares `credentialExpiry: "jwt-or-never"`: the minted key never expires and stale stored timestamps are ignored.
- **Transport**: `museCodeProvider.prepareRequest` / `prepareModelDiscovery` (`packages/ai/src/registry/muse-code.ts`) unwrap the minted key and add `x-api-version: 1.0.0`.
- **Usage**: `museCodeUsageProvider` (`packages/ai/src/usage/muse-code.ts`) re-reads the key endpoint (without onboarding) and maps `subs_usage.window` / `subs_usage.weekly` to percent limits. The endpoint rate-limits aggressively, so a failed refresh cools down for five minutes (`failureBackoffMs`).

### Catalog model handling
- **Descriptor & Management**: `muse-code` in `CATALOG_PROVIDERS` (`defaultModel: "muse-spark-1.3"`, `dynamicModelsAuthoritative`) uses `museCodeModelManagerOptions`: the Meta seed re-homed to `muse-code` with API-equivalent pricing, discovery with the `x-api-version` header, and a model cache keyed by the minted key and base URL.
...
- **Usage quota**: `minimaxCodeUsageProvider` in `packages/ai/src/usage/minimax-code.ts` polls `GET /v1/token_plan/remains` at `https://api.minimax.io` (or China equivalent), parsing rolling interval and weekly usage windows per plan bucket into remaining percentages for `proto usage`.
...
- **Custom Request Headers**: `getOpenRouterHeaders` in `packages/ai/src/utils/openrouter-headers.ts` attaches `User-Agent: proto/<ver>`, `HTTP-Referer: https://github.com/myanvoos/proto`, `X-OpenRouter-Title: proto`, `X-OpenRouter-Categories: cli-agent`, `X-OpenRouter-Cache: true`, and `X-OpenRouter-Cache-TTL: 3600` to all requests for edge response caching.
...
- **Authoritative Reported Cost Reconciling**: `applyProviderReportedCost` in `packages/ai/src/providers/openai-shared.ts` extracts `rawUsage.cost` echoed in API responses. If estimated token cost is finite and positive, input, output, cache-read, and cache-write costs are scaled by `reportedCost / estimatedCost` to match OpenRouter's exact billable total; otherwise, `usage.cost.input` is assigned the reported cost directly.
...
## SingularityAPI (`singularityapi-dev`, `singularityapi-tech`)
SingularityAPI ships two unrelated OpenAI Chat Completions products behind one brand, modeled as separate providers because they share no key, billing model, or effort ladder: the pay-as-you-go universal gateway `singularityapi-dev` (`https://api.singularityapi.dev/v1`, `sk-sapi-...` keys) and the slot-reserved DeepSeek lanes `singularityapi-tech` (`https://api.singularityapi.tech/v1`, `sk-...` keys).

### Special casings
- **Runtime-Only Catalog**: Both descriptors are `dynamicModelsAuthoritative: true` without `catalogDiscovery`, so bundle generation never freezes one account's roster; models are discovered live via `/v1/models` with a key.
- **Credential-Scoped Cache**: Rosters are issued per key, so `resolveModelCacheProviderId` hashes the credential plus the `/v1`-normalized endpoint (`normalizeSingularityApiBaseUrl`, `packages/catalog/src/wire/singularityapi.ts`) and both ids are in `CREDENTIAL_SCOPED_MODEL_CACHE_PROVIDERS`.
- **Gateway Wire Shape**: Every row sends `max_tokens`; lane rows also read top-level `reasoning_content`.
- **Measured Effort Ladders**: The wire publishes no reasoning metadata, so the resolvers assert the ladders measured live: dev DeepSeek V4 Flash/Pro `low/medium/high` (dev 400s `xhigh`/`max`), dev `gpt-5.6-sol/terra/luna` `low..max`, lane DeepSeek V4.1 Flash `low/high/xhigh/max` (lanes 400 `medium`). All use `none-effort` as the off switch; the resolver-authored ladders are kept verbatim rather than replaced by family defaults. Dev DeepSeek rows reject `tool_choice`.
- **Dev Capabilities**: `/v1/models` rows carry per-endpoint `capabilities` (context window, max output, per-million pricing strings); rows that serve only `/v1/images/generations` are dropped.
- **Lane Model Guide**: Lane ids (`deepseek-ai/DeepSeek-V4.1-Flash`, `deepseek-v4.1-flash`, dated `…-v4-flash-0731`) get a 262,144-token context window and image input.

### Auth & usage
- **API Key Login**: `loginSingularityApiDev` / `loginSingularityApiTech` (`packages/ai/src/registry/singularityapi-{dev,tech}.ts`) strip a pasted `Bearer ` prefix and validate against each product's non-billable `/v1/models`. Env vars: `SINGULARITYAPI_DEV_API_KEY`, `SINGULARITYAPI_TECH_API_KEY`.
- **Reservation Slots**: A valid lane key with no active reservation passes login but answers 403 at inference until a slot is booked.

## StepFun (`stepfun`)
StepFun's Open Platform (`https://api.stepfun.ai/v1`) serving its own chat models (`step-5-preview`, `step-3.7-flash`, `step-3.5-flash`, `step-3.5-flash-2603`). Transport: OpenAI Chat Completions. The China deployment (`api.stepfun.com`) and Step Plan endpoints are separate deployments whose keys are not interchangeable with `.ai` keys.

### Special casings
- **`max_tokens` only**: every row sends `maxTokensField: "max_tokens"`. StepFun documents `max_tokens` and silently ignores `max_completion_tokens`, which would leave output budgets unlimited.
- **Provider-owned effort ladder**: seeded rows carry `low/medium/high`; relay hosts advertise `minimal…xhigh` for the same models, but StepFun's own API rejects those tiers. Ids StepFun adds later take their ladder from `/v1/models` `reasoning_effort_support_list` (unknown tiers dropped); a row advertising none stays non-reasoning.
- **Non-chat roster filtering**: `/v1/models` interleaves StepAudio (TTS/ASR) and image SKUs with the chat models; `isStepfunChatModelId` drops the `stepaudio-`, `step-image-`, `step-tts-`, and `step-2x-large` prefixes.
- **No cross-host backfill**: the descriptor sets `skipCrossProviderReferenceFills`, so generation never copies relay-host limits or capabilities onto first-party rows.

### Auth & usage
- **API key**: `STEPFUN_API_KEY`. `/login stepfun` probes chat completions with `step-5-preview`; only a 401/403 rejects the key, other probe failures keep it.
- **Pricing**: a cache miss already includes writing the prefix to the cache, so rows carry `cacheWrite: 0`.

### Catalog model handling
- **Seeded, authoritative discovery**: `STEPFUN_STATIC_MODELS` (published model cards and list prices) keep the provider selectable without a key; a successful `/v1/models` snapshot replaces them, so a retired id leaves the picker.
...
### Auth & usage
- **Credential Resolution & Defaults**: Managed via `loginVllm` (`createApiKeyLogin` in `packages/ai/src/registry/vllm.ts`). Reads optional API keys from the `VLLM_API_KEY` environment variable or credentials stored via `proto auth-broker login vllm`.
...
- **OAuth flow & browser sign-in**: `zaiCodingPlanProvider` (`packages/ai/src/registry/zai.ts`) routes sign-in to `loginZaiOAuth` / `ZaiOAuthFlow` (`packages/ai/src/registry/oauth/zai.ts`). It initiates authorization at `AUTHORIZE_URL` (`https://chat.z.ai/api/oauth/authorize`) with ZCode's desktop redirect `zcode://zai-auth/callback` — Z.AI's allowlist rejects loopback redirects for the reused ZCode client — so the flow binds no callback server and the user pastes the `zcode://` URL or code (`ZAI_OAUTH_REDIRECT_URI` overrides the redirect) and exchanges authorization codes at `TOKEN_URL` (`https://zcode.z.ai/api/v1/oauth/token`).
