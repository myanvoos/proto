...
- timeout from `resolveMCPTimeoutMs`: `PROTO_MCP_TIMEOUT_MS` env override, else `config.timeout ?? 30000`; `0` disables
...
- timeout uses `AbortController` via `createMCPTimeout` (`PROTO_MCP_TIMEOUT_MS` override, else `config.timeout ?? 30000`; `0` disables)
