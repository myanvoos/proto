...
A tool that re-registers a built-in name (e.g. wrapping `bash` to add logging or a policy check) can
...
### File write/delete fallback

The current `ExtensionAPI` has no file-write or file-delete fallback registration seam; use `tool_call`/`tool_result` interception or a custom tool for policy over file mutation (which flows through bash and the kernel).
