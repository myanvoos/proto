 ...
 *
 * WHY THIS MODULE EXISTS. Five byte strings were declared sixteen times across `@oh-my-pi/pi-tui` and
 * `@oh-my-pi/pi-coding-agent` under eleven different names: `\x1b[0m` as `SEGMENT_RESET` twice, `RESET` twice and
 ...
 *
 * ONE DELIBERATE EXCEPTION. `@oh-my-pi/pi-coding-agent`'s `utils/qrcode.ts` keeps its own `ANSI_RESET`. That
 ...
 * declared three times in `@oh-my-pi/pi-coding-agent` as `BEL`, `OSC_TERMINATOR_BEL` and `SIXEL_END_BELL`, so the
 ...
 * parameter range, which is what `@oh-my-pi/pi-utils/strip-ansi` accepts for a general CSI. That
