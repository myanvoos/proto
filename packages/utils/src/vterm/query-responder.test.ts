...
test("reassembles a query split across chunks and answers several in order", () => {
	const responder = new TerminalQueryResponder();
	expect(responder.feed("prompt> \x1b[")).toBe("");
	expect(responder.feed("6n")).toBe("\x1b[1;1R");
	expect(responder.feed("\x1b]11;?\x07\x1b[6n")).toBe("\x1b]11;rgb:0000/0000/0000\x07\x1b[1;1R");
});
