...
describe("document conversion cache directory", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfile = process.env.PROTO_PROFILE;
	...
	afterEach(async () => {
		...
		restoreEnv("PROTO_PROFILE", originalOmpProfile);
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/proto exists", async () => {
		...
		expect(getDocumentConversionCacheDir()).toBe(
			path.join(process.env.XDG_CACHE_HOME, "proto", "cache", "document-conversions"),
		);
	});
...
describe("test directory state cleanup", () => {
	it("restores the active profile from the current env after setAgentDir mutations", () => {
		...
		const originalOmpProfile = process.env.PROTO_PROFILE;
		const originalPiProfile = process.env.PI_PROFILE;
		const originalXdgCacheHome = process.env.XDG_CACHE_HOME;
		try {
			process.env.PROTO_PROFILE = "cache-profile";
...
describe("legacy file adoption on XDG paths", () => {
	...
	it("adopts legacy files at the XDG paths without clobbering existing XDG files", async () => {
		...
		await fs.writeFile(path.join(xdgData, "proto", "marketplaces.json"), '{"xdg":true}');
		...
		expect(key).toBe(path.join(xdgState, "proto", "secret-placeholder.key"));
		expect(registry).toBe(path.join(xdgData, "proto", "marketplaces.json"));
