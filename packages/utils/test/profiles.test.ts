...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		configDir = `.proto-profile-test-${Snowflake.next()}`;
	...
	afterEach(async () => {
		...
		if (originalOmpProfileEnv === undefined) {
			delete process.env.PROTO_PROFILE;
		} else {
			process.env.PROTO_PROFILE = originalOmpProfileEnv;
		}
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "proto", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "proto", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "proto", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "proto", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "proto", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "proto", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "proto"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid PROTO profile");
		expect(() => setProfile("work/team")).toThrow("Invalid PROTO profile");
	});
...
describe("profile env + name validation", () => {
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid PROTO profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid PROTO profile");
...
describe("dirs module import behavior", () => {
	...
	it("exposes worker-host without loading agent env", async () => {
		...
		try {
			...
			await Bun.write(path.join(agentDir, ".env"), "PROTO_WORKER_HOST_PROBE=from-agent-env\n");
			const probePath = path.join(root, "probe.ts");
			await Bun.write(
				probePath,
				[
					...
					"process.stdout.write(JSON.stringify({",
					"	envProbe: process.env.PROTO_WORKER_HOST_PROBE ?? null,",
			...
			delete childEnv.PROTO_WORKER_HOST_PROBE;
			const proc = Bun.spawn([process.execPath, probePath], {
				stdout: "pipe",
				stderr: "pipe",
				env: childEnv,
	...
	it("ignores inherited profile agent dir when PROTO_PROFILE explicitly selects default", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-utils-dirs-default-profile-"));
		const probeConfigDir = `.proto-default-profile-${Snowflake.next()}`;
		try {
			const dirsUrl = url.pathToFileURL(path.join(import.meta.dir, "..", "src", "dirs.ts")).href;
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		const profileConfigDir = `.proto-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "proto", "profiles", "work");
