...
async function createFixture(order: readonly string[]): Promise<string> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "proto-stats-archive-"));
	tempDirs.push(root);
	for (const relativePath of order) {
		const filePath = path.join(root, relativePath);
		await fs.mkdir(path.dirname(filePath), { recursive: true });
		await Bun.write(filePath, relativePath === "index.html" ? "<main>PROTO</main>" : "body { color: blue; }");
	}
	return root;
}
...
describe("embedded stats client archive", () => {
	test("is byte-stable across filesystem order and carries zero timestamps", async () => {
		...
		expect(await files.get("index.html")?.text()).toBe("<main>PROTO</main>");
