...
- Added a Tools tab to the `proto stats` dashboard (`/#/tools`): per-tool call counts, error rates, result/argument payload sizes, per-model breakdown, and a stacked calls-over-time chart. Token and cost columns attribute each invoking turn's real provider usage evenly across that turn's tool calls. Existing databases re-parse sessions once on the next sync to backfill historical tool calls.
...
- Fixed application crashes and Bun aborts on macOS and when parsing large stats session files, including during `proto --smoke-test` runs, by utilizing a more resilient serial parser and lenient line scanner.
...
- Added a Gain tab to the `proto stats` dashboard (`/#/gain`) to display snapcompact token-savings with project scoping from synced session folders.

## [16.1.17] - 2026-06-24

### Fixed

- Stats sync counted the same provider request multiple times when a forked or branched session file copied the parent's entries verbatim. Inserts now skip rows whose `(entry_id, timestamp)` already exists under a different `session_file`, and a one-shot migration on the next `proto stats` run collapses any pre-existing duplicates ([#3370](https://github.com/can1357/oh-my-pi/issues/3370)).
...
- Redesigned the local stats dashboard with an PROTO-themed product shell, dedicated per-section views, accessible loading/empty/error states, and flicker-free navigation between screens and time ranges.
...
- Renamed `__proto_stats_sync_worker` to `__proto_worker_stats_sync`.
...
- The session-sync worker re-enters the host CLI entry (`workerHostEntry()` + `__proto_stats_sync_worker` argv selector) when running inside proto — source, npm bundle, or compiled binary — and keeps loading its own `sync-worker.ts` module directly for standalone `proto-stats`, bun test, and SDK hosts

## [15.1.6] - 2026-05-19

### Fixed

- Fixed `proto stats` crashing on first session sync in published `proto-{linux,darwin,windows}-*` binaries with `BuildMessage: ModuleNotFound resolving "./packages/stats/src/sync-worker.ts"`; the release build script now lists the stats sync, browser tab, and JS eval workers as explicit `--compile` entrypoints so Bun emits them into bunfs, matching the dev build script and the AGENTS.md worker spawn contract. ([#1150](https://github.com/can1357/oh-my-pi/issues/1150))
...
- Fixed `proto stats` in compiled binaries by using the serial sync path instead of spawning a raw file-asset worker that cannot import bundled parser code.
...
- Fixed `proto stats` failing to build the client on globally-installed installs by promoting `tailwindcss` from `devDependencies` to `dependencies` (the client build runs at runtime)
