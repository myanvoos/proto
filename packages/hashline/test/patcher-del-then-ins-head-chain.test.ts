...
import { formatHashlineHeader, InMemoryFilesystem, InMemorySnapshotStore, Patch, Patcher } from "@oh-my-pi/hashline";
...
describe("Patcher DEL then INS.HEAD chain", () => {
	it("clear file then insert new head content", async () => {
		...
		const r2 = await patcher.apply(Patch.parse(`${formatHashlineHeader("f.ts", tag)}\nINS.HEAD:\n+new`));
