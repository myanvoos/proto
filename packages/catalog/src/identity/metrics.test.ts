...
import { applyCatalogMetrics, CatalogMetricsIndex } from "./metrics";
...
const scored = new CatalogMetricsIndex([
	...
	model("anthropic", "claude-sonnet-4-5", { int: 48, tps: 60 }),
...
describe("CatalogMetricsIndex", () => {
	...
	test("does not cross product lines that share a revision", () => {
		...
		expect(scored.resolve(model("anthropic", "claude-sonnet-4-5-thinking"))).toBeUndefined();
	});
...
test("catalog metrics survive shared-catalog mapping", () => {
	...
	const mapped = models.find(candidate => candidate.provider === "anthropic" && candidate.id === "claude-opus-test");
	expect(mapped?.int).toBe(63.1);
	expect(mapped?.tps).toBe(56.6);
});
