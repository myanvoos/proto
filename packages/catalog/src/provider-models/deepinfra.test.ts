...
import { DEEPINFRA_BASE_URL, deepinfraModelManagerOptions } from "./openai-compat";
...
describe("DeepInfra built-in provider", () => {
	...
	test("keeps a live modality removal authoritative through the production manager merge", async () => {
		...
		// now text-only route.
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "proto-catalog-deepinfra-refresh-"));
