...
export const LEGACY_PI_MODULES_SPECIFIER = "proto-legacy-pi-modules";

const VIRTUAL_NAMESPACE = "proto-legacy-pi-modules-build";
...
export async function createLegacyPiVirtualModulePlugin(): Promise<Bun.BunPlugin> {
	...
	return {
		name: "proto:legacy-pi-modules",
		setup(build) {
			build.onResolve({ filter: /^proto-legacy-pi-modules$/ }, () => ({
