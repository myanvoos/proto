...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	if (requestedTools && !restrictToolNames) {
		...
		// Auto-learn's manage_skill is gated by `autolearn.enabled` but must also
		// be force-included into an explicit requestedTools list so a restricted
		// top-level session whose controller/guidance is active still exposes the
		// tool the nudge points at. Gated to top-level (taskDepth 0): the
		// controller only runs there, so a subagent's explicit tool whitelist
		// must never be silently widened with write-capable tools.
	...
	const isToolAllowed = (name: string) => {
		...
		// no goal record exists yet — /goal enables it so the agent can
