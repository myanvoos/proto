...
function parseOutline(source: string): OutlineNode[] | null {
	if (!supportsLanguage("python")) return null;
	const result = codeOutline({ code: source, lang: "python" });
...
export function renderPythonAstLines(source: string, theme: Theme, width: number): string[] | null {
	...
	return renderOutlineLines(root, theme, width, { headerKinds: PY_HEADER_KINDS, augKinds: PY_AUG_KINDS });
}
