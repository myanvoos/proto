...
__proto_sq_quote() {
	...
	__proto_qout=
	__proto_sq=\'
	while case "$__proto_qbuf" in *$__proto_sq*) true ;; *) false ;; esac; do
		__proto_qout=$__proto_qout${__proto_qbuf%%"$__proto_sq"*}"'\\''"
		__proto_qbuf=${__proto_qbuf#*"$__proto_sq"}
	done
	__proto_qout=$__proto_qout$__proto_qbuf
	printf "'%s'" "$__proto_qout"
}
...
__proto_emit_export_for() {
	...
	__proto_xv=
	eval "__proto_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__proto_sq_quote "$__proto_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__proto_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __proto_name; do
			__proto_emit_export_for "$__proto_name"
		done
}
