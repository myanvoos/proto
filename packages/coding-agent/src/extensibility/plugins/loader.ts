 ...
 *
 * `home` controls which `<plugins>/proto-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .proto and .pi directories).
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/proto-plugins.lock.json#plugins`.
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.proto/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.proto/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `proto-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `proto`/`pi` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `proto`/`pi`
 ...
 *   1. the directory's own package.json `proto`/`pi` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `proto`/`pi`
 ...
 *     and PROTO's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `proto`/`pi` `extensions` manifest are
