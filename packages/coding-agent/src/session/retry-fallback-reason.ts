...
/** Describe the health snapshot that caused a usage-preflight model switch. */
export function describeUsageFallback(health: ModelUsageHealth, reservePercent: number): string {
	...
	for (const account of health.accounts) {
		...
		if (reset !== undefined && Number.isFinite(reset) && reset > now && reset < earliestReset) earliestReset = reset;
	}
