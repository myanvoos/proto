...
class SqlSessionStorageBackend implements SessionStorageBackend {
	...
	async writeFull(
		...
		// MySQL reports `affectedRows: 0` for a matched-but-unchanged row, so a byte-identical replace looks unwritten;
		// only a real size divergence is a conflict.
	...
	async move(src: string, dst: string, mtimeMs: number): Promise<void> {
		// Deleting the destination first would delete the source itself on a same-path move.
		if (src === dst) {
			if ((await this.readFull(src)) === null) throw enoent(src);
			return;
		}
		// One transaction: a failed or missing source must not leave the destination deleted.
