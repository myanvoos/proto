...
export default class Say extends Command {
	...
	static examples = [
		'proto say "hello world"',
		"proto say --file notes.md --voice bm_fable",
		'proto say "hello world" --out /tmp/hello.wav',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`proto setup speech\` to install it.\n`,
			),
		);
	}
}
