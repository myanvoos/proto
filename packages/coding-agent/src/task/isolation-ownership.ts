/**
 * Ownership marker for task-isolation sandboxes under `~/.proto/wt/`.
 *
 * Each isolation base dir (`ensureIsolation` in {@link ./worktree}) holds a
 * compact `m` mount plus this marker file naming the proto process that created
 * it. `proto worktree clear` consults the marker so it can distinguish a live
...
interface IsolationOwner {
	/** PID of the proto process that created and owns the sandbox. */
	pid: number;
 ...
 * `proto worktree clear` never sees an owner-less sandbox mid-creation.
 */
...
/**
 * Whether a live proto process still owns the sandbox at `baseDir`.
 *
