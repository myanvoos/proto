...
describe("TranscriptContainer", () => {
	...
	it("reflows an unacknowledged offer before narrow history can truncate its content", () => {
		const transcript = new TranscriptContainer();
		transcript.addChild(new ReflowingAppendBlock());
		const wide = transcript.peekFinalizedBatch(8, 0)!;
		expect(wide.rows).toEqual(["abcdefgh"]);
		const narrow = transcript.peekFinalizedBatch(2, 0)!;
		expect(narrow.rows).toEqual(["ab", "cd", "ef", "gh"]);
		expect(narrow.id).toBeGreaterThan(wide.id);
		transcript.acknowledgeFinalizedBatch(wide.id);
		expect(transcript.peekFinalizedBatch(2, 0)?.id).toBe(narrow.id);
		transcript.acknowledgeFinalizedBatch(narrow.id);
	...
	it("counts multi-row snapshot prefixes by rendered rows, not snapshot count", () => {
		...
		transcript.liveRowCount(2);
		const batch = transcript.peekFinalizedBatch(2, 1)!;
		expect(batch.rows).toEqual(["ab", "cd", "ef", "gh"]);
	...
	it("drops emitted stable rows on reset so a replay honors a hidden presentation", () => {
		const transcript = new TranscriptContainer();
		...
		expect(replay.rows).toEqual(["answer", ""]);
	...
	it("excludes empty blocks so pressure never emits blank rows", () => {
		const transcript = new TranscriptContainer();
	...
	it("empty blocks do not reserve capacity from real text under pressure", () => {
		const transcript = new TranscriptContainer();
	...
	it("gives surplus rows to assistant text before a growing tool card", () => {
		const transcript = new TranscriptContainer();
