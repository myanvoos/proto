...
export class AgentFleetOverlayComponent extends Container implements SelectListMouseTarget {
	#registry: AgentRegistry;
	#observers: SessionObserverRegistry;
	...
	constructor(deps: AgentFleetDeps) {
		super();
		...
		if (this.#rows.length === 0) {
			if (this.#loadingPersistedSubagents) {
				...
				const emptyState = [
					...
					theme.fg("dim", "Resume that session with proto --continue, or spawn a worker here."),
				];
