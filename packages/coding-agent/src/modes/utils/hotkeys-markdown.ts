...
export function buildHotkeysMarkdown(bindings: HotkeysMarkdownBindings): string {
	...
	return [
		...
		`| \`${appKey(bindings, "app.agents.fleet")}\` / \`${appKey(bindings, "app.session.observe")}\` | Open the agent fleet |`,
		"| double-tap `←` (empty editor) | Open the session switcher (`/sessions`); while viewing an agent, hop to its parent |",
		"| double-tap `→` (empty editor) | Open the subagents of the session or agent you are viewing (`/agents`) |",
		"| `Esc` (empty editor, viewing an agent) | Return to the main session |",
