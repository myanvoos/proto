...
- Documents → extracted text. Notebooks → editable cells. Images → {{#if INSPECT_MEDIA_ENABLED}}metadata; call `inspect_media`{{else}}decoded inline{{/if}}. `:raw` bypasses converters.
...
- `ssh://host/<path>` reads remote file/dir (UTF-8, ≤1 MiB); bare `ssh://` lists hosts; writable with `write`.
