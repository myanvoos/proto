...
- Directory → depth-limited listing. Documents → text; notebooks → editable cells; images → {{#if INSPECT_MEDIA_ENABLED}}metadata (use `inspect_media`){{else}}decoded inline{{/if}}; `:raw` bypasses converters.
