...
{{#if hasKernelBridge}}- `python`/`node` with code on stdin (heredoc/pipe) or bare `-c`/`-e CODE` run in the persistent eval kernels — same state, helpers (`write`, `edit`, `agent`, …), and stale-write guard as `eval`; composes in pipelines. `python file.py`, `-m`, or extra argv runs a real fresh interpreter.
- Orchestration default: write the script (`agent()`, `parallel()`, `pipeline()`) to `fleet://<name>.py` (heredoc redirect or the write tool), then run `python fleet://<name>.py` — scripts under `fleet://` execute in the kernel, everything else under a path runs real interpreters.{{/if}}
