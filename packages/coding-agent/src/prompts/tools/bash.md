...
Read first; localized replacements MUST assert anchors and occurrence counts before writing. Every mutation is tracked and diffed; stale writes raise `StaleWriteError` before truncation, so re-read and redo the edit.{{#if py}} Python `NAME = <<DELIMITER` heredoc assignments protect quote-hostile literal payloads inside the kernel cell; native Python strings remain the default.{{/if}}
