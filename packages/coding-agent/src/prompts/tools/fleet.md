...
- NEVER inspect other sessions' files to figure out what a peer is doing. Message them directly.
- NEVER use fleet messaging for something a tool can answer (e.g., grepping codebase, running a build).

# Processes

Project-scoped long-running processes shared by every proto instance in the same directory. A long-running service, watcher, debugger, REPL, or process needing later input MUST use `op:"start"`, not `bash`.
...
- **`start`** launches `application` + `args` directly. `cwd` defaults to the session directory; `pty` defaults true.
  ...
  - `persist: true` opts out of last-proto teardown; `detached: true` survives broker shutdown and all proto exits (implies persist, disables PTY input). Omit both unless their survival guarantees are required.
