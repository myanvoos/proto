...
{{#if py}}bash(command, timeout?=None, cwd?=None, env?=None) → BashResult
    Run a shell command; await the handle. Merged stdout+stderr in `.output`, exit in `.exit_code`. `os.chdir`/`os.environ` persist between calls. Unawaited handles keep running; poll `.poll()`/`.output()`, stop with `.kill()`.
...
block_range(path, line) → (start, end) | None
    Tree-sitter syntactic block extent containing 1-based `line`.
edit_block(path, line, body, expect) → Path
    Replace the syntactic block containing `line` with `body`; refuses unless `expect` equals current content.{{/if}}
