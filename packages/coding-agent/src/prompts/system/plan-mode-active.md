...
<procedure>
1. **Understand** — request and supporting code.{{#if scoutAvailable}}{{#if workerAvailable}} Scope spans areas → parallel `scout` subagents via `orchestrate_spawn`, distinct focuses: implementations, related components, test patterns.{{/if}}{{/if}} Find reusable code before proposing new.
