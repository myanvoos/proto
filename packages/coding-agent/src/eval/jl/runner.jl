# Proto Julia runner — subprocess wrapper used by the coding-agent host.
...
function next_drain_marker()
    ...
    return Vector{UInt8}(codeunits("\0__PROTO_DRAIN__:" * string(drain_marker_counter[]) * ":" * string(time_ns()) * "\0"))
end
