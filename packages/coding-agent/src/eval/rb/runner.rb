...
# Proto Ruby runner — subprocess wrapper used by the coding-agent host.
#
...
$__proto_out_mutex = Mutex.new
$__proto_raw_stderr = (STDERR.dup rescue STDERR)
$__proto_current_rid = nil
$__proto_capture_rid = nil
$__proto_exec_count = 0
$__proto_active_exec = 0
$__proto_silent = false

begin
  $__proto_frame_io = STDOUT.dup
  $__proto_frame_io.sync = true
  __proto_stdout_cap_r, __proto_stdout_cap_w = IO.pipe
  STDOUT.reopen(__proto_stdout_cap_w)
  STDOUT.sync = true
  __proto_stdout_cap_w.close
  $__proto_stdout_capture_read = __proto_stdout_cap_r
rescue StandardError
  $__proto_frame_io = STDOUT
  ($__proto_frame_io.sync = true) rescue nil
  $__proto_stdout_capture_read = nil
end

begin
  __proto_stderr_cap_r, __proto_stderr_cap_w = IO.pipe
  STDERR.reopen(__proto_stderr_cap_w)
  STDERR.sync = true
  __proto_stderr_cap_w.close
  $__proto_stderr_capture_read = __proto_stderr_cap_r
rescue StandardError
  $__proto_stderr_capture_read = nil
end
...
begin
  $__proto_proto_stdin = STDIN.dup
  STDIN.reopen(File.open(File::NULL, "r"))
rescue StandardError
  $__proto_proto_stdin = STDIN
end
...
def __proto_emit(frame)
  ...
  $__proto_out_mutex.synchronize do
    $__proto_frame_io.write(line)
    $__proto_frame_io.write("\n")
    $__proto_frame_io.flush
  end
rescue StandardError
  nil
end
...
def __proto_emit_stream(kind, text)
  rid = $__proto_current_rid
  if rid.nil?
    ($__proto_raw_stderr.write(text) rescue nil)
    return
  end
  __proto_emit("type" => kind, "id" => rid, "data" => __proto_scrub(text))
end

def __proto_emit_display(bundle, kind = "display")
  rid = $__proto_current_rid
  return if rid.nil?
  __proto_emit("type" => kind, "id" => rid, "bundle" => bundle)
end

def __proto_emit_status(op, data = {})
  status = { "op" => op.to_s }
  data.each { |k, v| status[k.to_s] = v }
  __proto_emit_display({ "application/x-proto-status" => status }, "display")
end

PROTO_IMAGE_MIMES = %w[image/png image/jpeg].freeze
...
def __proto_image_payload(content)
  require "base64"
  s = content.to_s
  return s.gsub(/\s+/, "") if __proto_base64?(s)
...
def __proto_sniff_image_mime(bytes)
  b = bytes.to_s.b
...
def __proto_normalize_bundle(hash)
  bundle = {}
  hash.each do |key, val|
    k = key.to_s
    bundle[k] =
      if PROTO_IMAGE_MIMES.include?(k)
        __proto_image_payload(val)
...
def __proto_finalize_bundle(bundle, value)
  bundle["text/plain"] ||= __proto_scrub((value.inspect rescue value.class.name))
  bundle
end
...
def __proto_rich_mime_bundle(value)
  if value.respond_to?(:to_proto_mime)
    mime = (value.to_proto_mime rescue nil)
    return __proto_finalize_bundle(__proto_normalize_bundle(mime), value) if mime.is_a?(Hash) && !mime.empty?
  end
  if value.respond_to?(:to_iruby_mimebundle)
    ...
    return __proto_finalize_bundle(__proto_normalize_bundle(data), value) if data.is_a?(Hash) && !data.empty?
  end
  if value.respond_to?(:to_iruby)
    pair = (value.to_iruby rescue nil)
    if pair.is_a?(Array) && pair.size == 2 && !pair[0].nil?
      return __proto_finalize_bundle(__proto_normalize_bundle({ pair[0].to_s => pair[1] }), value)
    end
  end
  ...
  if value.respond_to?(:to_png)
    ...
    return __proto_finalize_bundle({ "image/png" => __proto_image_payload(png) }, value) if png
  end
  jpeg_method = %i[to_jpeg to_jpg].find { |m| value.respond_to?(m) }
  if jpeg_method
    jpg = (value.public_send(jpeg_method) rescue nil)
    return __proto_finalize_bundle({ "image/jpeg" => __proto_image_payload(jpg) }, value) if jpg
  end
  if value.respond_to?(:to_blob)
    blob = (value.to_blob rescue nil)
    if blob.is_a?(String) && (mime = __proto_sniff_image_mime(blob))
      return __proto_finalize_bundle({ mime => __proto_image_payload(blob) }, value)
    end
  end
...
def __proto_mime_bundle(value)
  case value
  when String
    { "text/plain" => __proto_scrub(value) }
  when Hash, Array
    safe = begin
    ...
    if safe.nil?
      { "text/plain" => __proto_scrub(value.inspect) }
    else
      { "application/json" => safe, "text/plain" => __proto_scrub(value.inspect) }
    end
  when nil
    { "text/plain" => "nil" }
  else
    __proto_rich_mime_bundle(value) || { "text/plain" => __proto_scrub(value.inspect) }
  end
end

def __proto_present(value, kind = "display")
  __proto_emit_display(__proto_mime_bundle(value), kind)
end
...
def __proto_start_capture_drain(io, kind)
  return if io.nil?
  Thread.new do
    loop do
      chunk =
      ...
      rid = $__proto_capture_rid
      if rid.nil?
        ($__proto_raw_stderr.write(chunk) rescue nil)
      else
        __proto_emit("type" => kind, "id" => rid, "data" => __proto_scrub(chunk))
      end
    end
  end
end

def __proto_start_parent_watchdog
...
def __proto_install_idle_sigint
  Signal.trap("INT", "IGNORE") rescue nil
end

def __proto_install_exec_sigint
  Signal.trap("INT", "DEFAULT") rescue nil
end

def __proto_begin_exec
  $__proto_active_exec += 1
  __proto_install_exec_sigint
end

def __proto_end_exec
  $__proto_active_exec -= 1 if $__proto_active_exec > 0
  __proto_install_idle_sigint if $__proto_active_exec.zero?
end
...
def __proto_apply_request_runtime(req)
  cwd = req["cwd"]
  ...
  env = req["env"]
  if env.is_a?(Hash)
    PROTO_MANAGED_ENV_KEYS.each do |key|
...
def __proto_should_display_result?(src)
  ...
  last = __proto_ast_last(node)
  return true if last.nil?
  !PROTO_NON_DISPLAY_NODES.include?(last.type)
end
...
def __proto_emit_error(rid, exc, name_override = nil)
  ...
  __proto_emit(
    ...
    "id" => rid,
    "ename" => ename,
    "evalue" => __proto_scrub(evalue),
    "traceback" => traceback.map { |l| __proto_scrub(l) },
  )
end

def __proto_handle_request(req)
  rid = req["id"].to_s
  $__proto_current_rid = rid
  $__proto_capture_rid = rid
  $__proto_silent = req["silent"] == true
  $__proto_exec_count += 1
  count = $__proto_exec_count
  __proto_emit("type" => "started", "id" => rid)

  status = "ok"
  cancelled = false
  begin
    begin
      __proto_apply_request_runtime(req)
      src = req["code"].to_s
    rescue Exception => e # rubocop:disable Lint/RescueException
      __proto_emit_error(rid, e)
      __proto_emit("type" => "done", "id" => rid, "status" => "error", "executionCount" => count, "cancelled" => false)
      return
    end
    ...
    begin
      ...
      unless $__proto_silent || value.nil? || !__proto_should_display_result?(src)
        __proto_present(value, "result")
      end
    rescue Interrupt => e
      cancelled = true
      status = "error"
      __proto_emit_error(rid, e, "Interrupt")
    rescue SystemExit => e
      status = "error"
      __proto_emit_error(rid, e)
    rescue Exception => e # rubocop:disable Lint/RescueException
    ...
    __proto_emit("type" => "done", "id" => rid, "status" => status, "executionCount" => count, "cancelled" => cancelled)
  ensure
    $__proto_capture_rid = nil if $__proto_capture_rid == rid
    $__proto_current_rid = nil
  end
end
...
def __proto_main
  ...
  __proto_install_idle_sigint
  __proto_start_parent_watchdog
  __proto_start_capture_drain($__proto_stdout_capture_read, "stdout")
  __proto_start_capture_drain($__proto_stderr_capture_read, "stderr")

  $__proto_proto_stdin.each_line do |raw|
    line = raw.strip
    next if line.empty?
    req =
      begin
    ...
    __proto_handle_request(req) if req.is_a?(Hash)
  end
end
