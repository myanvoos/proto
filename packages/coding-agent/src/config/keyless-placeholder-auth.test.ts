...
beforeEach(() => {
	...
	authRequired = true;
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "proto-keyless-placeholder-"));
});
...
function bootRegistry(storage: AuthStorage): ModelRegistry {
	// Isolated settings: the global singleton may carry a developer's `disabledProviders`.
...
describe("keyless empty-paste placeholder vs. wire auth", () => {
