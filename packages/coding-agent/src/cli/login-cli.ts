/**
 * `proto login [provider]` — the terminal counterpart of the in-session `/login`.
 *
 * Authenticates against the same credential store sessions read (local `agent.db`, or the configured auth
 * broker), including OAuth providers contributed by extensions, then re-runs that provider's model discovery
 * so the next session sees the models the credential unlocked.
 */
...
/** Unknown providers, cancelled prompts, and failed flows print `Login failed: …` and set exit code 1. */
export async function runLoginCommand(provider: string | undefined): Promise<void> {
	const cwd = getProjectDir();
	const settings = await Settings.init({ cwd });
	const authStorage = await discoverAuthStorage(undefined, { settings, cwd });
	let rl: readline.Interface | undefined;
	try {
		const modelRegistry = new ModelRegistry(authStorage);
		// Extensions may register OAuth providers; load them so they are listed.
		await loadCliExtensionProviders(modelRegistry, settings, cwd);
		// Created only once prompts can follow: piped lines emitted with no pending question are dropped.
		...
		// A fresh cache row fetched before login would otherwise hide the unlocked models until its TTL expires.
