...
describe("SSH control-path budget (#9070)", () => {
	it("rejects a control dir that overflows sun_path once %C.sock + mux temp bind is added", () => {
		...
		const profileDir = "/Users/arthur/.proto/profiles/upstream/ssh-control";
		...
		expect(controlPathFitsBudget("/Users/arthur/.proto/ssh-control", "darwin")).toBe(true);
	});
...
describe("sshControlFallbackDir", () => {
	it("is deterministic and leaves 11 bytes of macOS sun_path slack", () => {
		...
		expect(a).toBe(b);
		expect(a).toBe("/tmp/proto-5434354bc38f9a50fbbd");
	...
	it("isolates distinct canonical control directories and uids", () => {
		const base = "/Users/arthur/.proto/ssh-control";
		expect(sshControlFallbackDir(base, 501)).not.toBe(
			sshControlFallbackDir("/different/xdg/state/proto/ssh-control", 501),
		);
...
describe("resolveSshControlDir", () => {
	it("keeps the canonical dir when it fits", () => {
		const canonicalDir = "/Users/arthur/.proto/ssh-control";
	...
	it("relocates to the bounded shared fallback when the canonical dir overflows", () => {
		...
		expect(choice).toEqual({ dir: "/tmp/proto-5434354bc38f9a50fbbd", shared: true });
	...
	it("keeps distinct fallback masters for the same profile under different XDG state roots", () => {
		const a = resolveSshControlDir({
			canonicalDir: "/very/long/xdg-state-a/proto/profiles/upstream/ssh-control",
			platform: "darwin",
			uid: 501,
		});
		const b = resolveSshControlDir({
			canonicalDir: "/very/long/xdg-state-b/proto/profiles/upstream/ssh-control",
			platform: "darwin",
			uid: 501,
		});
...
describe("assertOwnerPrivateDir", () => {
	...
	const mkScratch = () => {
		scratch = fs.mkdtempSync(path.join(os.tmpdir(), "proto-ssh-guard-"));
		return scratch;
	};
