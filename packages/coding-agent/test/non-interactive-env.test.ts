...
it("filters expanded dotenv values while preserving matching and empty launcher values", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "proto-env-"));
	try {
		await Bun.write(
			path.join(tmp, ".env"),
			[
				"BASE=loaded-by-proto",
		...
		const script = [
			...
			"	inherited: env.PROTO_TEST_INHERITED_MARKER ?? null,",
		...
		for (const bunArgs of bunArgSets) {
			const proc = Bun.spawn([process.execPath, ...bunArgs, "--no-install", "--eval", script], {
				...
				env: {
					...
					EMPTY_PARENT_VAR: "",
					PROTO_TEST_INHERITED_MARKER: "keep-me",
					NODE_ENV: "development",
