...
it("prints the embedded PROTO license and aggregate notices on the exact top-level flag path", async () => {
	...
	expect(stderr).toBe("");
	expect(stdout).toBe(`PROTO License and Third-Party Notices\n\n${license.trimEnd()}\n\n${notices.trimEnd()}\n`);
});
