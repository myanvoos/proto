...
const ORIGINAL_PROTO_PLUGIN_DIR = process.env.PROTO_PLUGIN_DIR;
const ORIGINAL_PROTO_MARKETPLACE_DIR = process.env.PROTO_MARKETPLACE_DIR;
...
afterEach(async () => {
	vi.restoreAllMocks();
	if (ORIGINAL_PROTO_PLUGIN_DIR === undefined) {
		delete process.env.PROTO_PLUGIN_DIR;
	} else {
		process.env.PROTO_PLUGIN_DIR = ORIGINAL_PROTO_PLUGIN_DIR;
	}
	if (ORIGINAL_PROTO_MARKETPLACE_DIR === undefined) {
		delete process.env.PROTO_MARKETPLACE_DIR;
	} else {
		process.env.PROTO_MARKETPLACE_DIR = ORIGINAL_PROTO_MARKETPLACE_DIR;
	}
...
describe("DAP adapter configuration", () => {
	it("loads a custom adapter from dap.json and selects it by file extension", async () => {
		const cwd = await makeTempDir("proto-dap-config-json-");
	...
	it("merges partial user overrides over built-in adapters", async () => {
		const cwd = await makeTempDir("proto-dap-config-override-");
	...
	it("loads adapter config from project config directories and YAML", async () => {
		const cwd = await makeTempDir("proto-dap-config-yaml-");
	...
	it("loads plugin DAP adapters from plugin config files", async () => {
		const cwd = await makeTempDir("proto-dap-config-plugin-");
		...
		process.env.PROTO_PLUGIN_DIR = path.join(cwd, "plugins");
		process.env.PROTO_MARKETPLACE_DIR = path.join(cwd, "marketplaces");
	...
	it("ignores invalid custom adapters without discarding valid configs", async () => {
		const cwd = await makeTempDir("proto-dap-config-invalid-");
		await fs.writeFile(
			path.join(cwd, "dap.json"),
			JSON.stringify({
				adapters: {
	...
	it("reports missing dlv for Go source instead of falling back to a native debugger", async () => {
		const cwd = await makeTempDir("proto-dap-go-source-missing-");
	...
	it("reports missing dlv for Go package directories instead of selecting a native debugger", async () => {
		const cwd = await makeTempDir("proto-dap-go-directory-missing-");
		const missingCommand = await setupMissingDlvProject(cwd);
	...
	it("prefers a nested module adapter over cwd and PATH for inferred launches", async () => {
		const cwd = await makeTempDir("proto-dap-go-nested-local-");
	...
	it("uses a nested module adapter when dlv is requested explicitly", async () => {
		const cwd = await makeTempDir("proto-dap-go-nested-explicit-");
		const { moduleRoot, program } = await setupNestedGoProgram(cwd);
		const nestedDlv = path.join(moduleRoot, "bin", "dlv");
		await writeExecutable(nestedDlv);
		const whichSpy = vi.spyOn(piUtils, "$which").mockReturnValue(path.join(cwd, "global", "dlv"));
	...
	it("prefers the session cwd adapter over PATH after a nested-root miss", async () => {
		const cwd = await makeTempDir("proto-dap-go-nested-cwd-");
	...
	it("resolves a local dlv for Go workspaces rooted by go.work", async () => {
		const cwd = await makeTempDir("proto-dap-go-work-");
