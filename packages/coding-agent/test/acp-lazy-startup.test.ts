...
describe("ACP lazy startup", () => {
	...
	it("honors explicit host-defaulted and todo settings for protocol hosts", async () => {
		...
		const explicit = {
			"orchestrator.isolation.mode": "rcopy",
			"orchestrator.isolation.apply": false,
			"orchestrator.isolation.merge": "branch",
			"orchestrator.isolation.commits": "ai",
			"orchestrator.maxConcurrency": 4,
			"orchestrator.maxRecursionDepth": 5,
