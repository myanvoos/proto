...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "proto-fetcher-test-"));
	});
	...
	it("loads catalog from .proto-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "proto-only");
		fs.mkdirSync(path.join(root, ".proto-plugin"), { recursive: true });
		const catalog = {
			name: "proto-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "proto-plugin", source: "./plugins/proto-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".proto-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("proto-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("proto-plugin");
	});

	it("prefers .proto-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".proto-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const ompCatalog = {
			name: "from-proto-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".proto-plugin", "marketplace.json"), JSON.stringify(ompCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-proto-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .proto-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.proto-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
