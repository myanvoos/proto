...
describe("substitutePluginRoot", () => {
	...
	it("replaces PROTO_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${PROTO_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${PROTO_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
