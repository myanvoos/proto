...
describe.skipIf(!supportsReftable)("git reftable support", () => {
	...
	beforeAll(async () => {
		...
		sharedRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "proto-reftable-"));
		...
		worktreeDir = await fs.mkdtemp(path.join(os.tmpdir(), "proto-reftable-wt-"));
		...
		configRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "proto-reftable-cfg-"));
