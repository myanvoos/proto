...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to PROTO-native only by migrating reusable shared AI instructions into .proto/AGENTS.md, .proto/rules, .proto/skills, and .proto/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .proto while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand PROTO resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand PROTO resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.proto without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Proto in terminal",
				description:
