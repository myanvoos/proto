...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		// resolves into `<tempHome>/.proto/plugins` on every platform. Two things are needed:
		//  - mock os.homedir() so configRoot = `<tempHome>/.proto` (the previous
		//    XDG_DATA_HOME redirect was a no-op on Windows, where these tests then wrote
		//    into and rm'd the developer's real `~/.proto/plugins`);
		//  - clear the XDG_* vars, because on Linux/macOS the resolver prefers
		//    `$XDG_DATA_HOME/proto` over the home config root when that dir exists, so an
		...
		// ~/.proto must fail loudly here instead of clobbering the developer's plugins.
