...
//    or `../`-relative entry pointing somewhere else under `~/.proto/`), plus
//    the legacy non-XDG `~/.proto/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.proto/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/proto/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.proto` when
...
// symlinks `~/.proto/agent/extensions/foo.ts` to a dotfiles repo would see the
