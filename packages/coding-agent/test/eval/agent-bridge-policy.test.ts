...
describe("agent() through eval runtimes", () => {
	...
	it("exposes agent() in JavaScript and parses structured output", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-js-");
	...
	it("bounds JavaScript parallel() by the orchestrator.maxConcurrency setting while preserving order", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-js-parallel-");
	...
	it("propagates JavaScript parallel() rejections", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-js-reject-");
	...
	it("exposes agent() in the Python runtime", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-py-");
	...
	it("bounds Python parallel() by the orchestrator.maxConcurrency setting while preserving order", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-py-parallel-");
		const settings = Settings.isolated({
			"async.enabled": false,
			"orchestrator.isolation.mode": "none",
			"orchestrator.enableLsp": true,
	...
	it("interrupting a Python parallel() fan-out aborts in-flight subagents and preserves session state", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-py-interrupt-");
		const settings = Settings.isolated({
			"async.enabled": false,
			"orchestrator.isolation.mode": "none",
			"orchestrator.enableLsp": true,
	...
	it("streams enriched agent progress through onStatus before the cell finishes", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-progress-");
	...
	it("pauses the idle watchdog while a quiet agent() runs past the budget", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-timeout-pause-");
	...
	it("keeps timeout paused despite agent() progress snapshots", async () => {
		using tempDir = TempDir.createSync("@proto-eval-agent-progress-timeout-pause-");
	...
	it("interrupting a JavaScript agent() aborts it at once but waits out its critical phase", async () => {
		...
		using tempDir = TempDir.createSync("@proto-eval-agent-js-interrupt-");
...
describe("runEvalAgent isolation", () => {
	...
	it("preserves temp artifacts for non-isolated handle outputs", async () => {
		...
		const removedArtifactsDir = rmSpy.mock.calls.some(
			([target]) => typeof target === "string" && target.includes("proto-eval-agent-"),
		);
	...
	it("throws on apply failure for non-schema callers too instead of burying the warning in text", async () => {
		...
		vi.spyOn(isolationRunner, "runIsolatedSubprocess").mockImplementation(async opts =>
			singleResult(opts.baseOptions, {
				output: "ran",
				branchName: `proto/task/${opts.agentId}`,
			}),
		);
		vi.spyOn(isolationRunner, "mergeIsolatedChanges").mockResolvedValue({
			summary: "\n\n<system-notification>Branch merge failed: proto/task/x.\nConflict: foo.ts</system-notification>",
			changesApplied: false,
			hadAnyChanges: false,
			mergedBranchForNestedPatches: false,
		});
		...
		const session = isolatedSession({ "orchestrator.isolation.merge": "branch" });
		await expect(runEvalAgent({ prompt: "scout", isolated: true }, { session })).rejects.toThrow(
			/isolated apply failed.*Branch merge failed.*Captured branch preserved as proto\/task\//s,
		);
	});
	...
	it("surfaces a captured branch name when apply=false and the run used branch mode", async () => {
		...
		expect(mergeSpy).not.toHaveBeenCalled();
		expect(result.details.branchName).toMatch(/^proto\/task\//);
		expect(result.text).toContain("proto/task/");
		expect(result.text).toContain("apply=false");
	});
