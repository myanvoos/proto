...
it("imports the CLI entry graph without loading dotenv before profile bootstrap", async () => {
	using tempDir = TempDir.createSync("@proto-js-process-import-");
	await Bun.write(path.join(tempDir.path(), ".env"), "PROTO_PROCESS_ENTRY_ENV_PROBE=loaded-too-early\n");
	const env = Object.fromEntries(
		Object.entries(process.env).filter((entry): entry is [string, string] => typeof entry[1] === "string"),
	);
	delete env.PROTO_PROCESS_ENTRY_ENV_PROBE;
...
async function pingComputerWorker(
	entry: string,
	id: string,
	argv: string[] = ["__proto_worker_computer"],
): Promise<unknown> {
