/**
 * Regression: the agent fleet row order must be stable while the fleet is open.
 *
 * The fleet is sorted by lastActivity on first open, but after that keyboard
 * selection must not jump around as agents heartbeat or update activity. New
 * agents that appear while the fleet is open are appended at the end.
 */
...
} from "@oh-my-pi/pi-coding-agent/modes/components/agent-fleet";
...
function makeHub(agents: AgentRegistry, overrides: Partial<AgentFleetDeps> = {}) {
	return new AgentFleetOverlayComponent({
...
function renderedAgentRows(hub: AgentFleetOverlayComponent, width = 120): RenderedAgentRow[] {
...
function renderedAgentIds(hub: AgentFleetOverlayComponent): string[] {
	return renderedAgentRows(hub).map(row => row.id);
}

function selectedAgentId(hub: AgentFleetOverlayComponent): string | undefined {
...
function renderedRosterHeaderLineRaw(hub: AgentFleetOverlayComponent, id: string, width: number): string {
...
describe("Agent fleet row ordering", () => {
	...
	it("freezes the initial lastActivity order while the fleet is open", () => {
		vi.useFakeTimers();
		let hub: AgentFleetOverlayComponent | undefined;
		try {
			geometry = stubStdoutGeometry(120);
	...
	it("opens the selected-agent inspector as a narrow-terminal fallback", () => {
		...
		try {
			...
			expect(details).toContain("Agent Fleet · NarrowAgent");
