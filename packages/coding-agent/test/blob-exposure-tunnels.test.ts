...
function prepareFake(output: string, options: { exitCode?: number; restartOnce?: boolean } = {}): FakeInvocation {
	...
	process.env.PROTO_FAKE_TUNNEL_ARGS = argsFile;
	process.env.PROTO_FAKE_TUNNEL_RUNS = runsFile;
	process.env.PROTO_FAKE_TUNNEL_SIGNALS = signalsFile;
	process.env.PROTO_FAKE_TUNNEL_OUTPUT = output;
	if (options.exitCode === undefined) delete process.env.PROTO_FAKE_TUNNEL_EXIT_CODE;
	else process.env.PROTO_FAKE_TUNNEL_EXIT_CODE = String(options.exitCode);
	const restartMarker = options.restartOnce ? path.join(fakeBinDir, `restart-${suffix}.txt`) : undefined;
	if (restartMarker === undefined) delete process.env.PROTO_FAKE_TUNNEL_RESTART_MARKER;
	else process.env.PROTO_FAKE_TUNNEL_RESTART_MARKER = restartMarker;
...
beforeAll(() => {
	fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), "proto-blob-tunnels-"));
	const target = path.join(fakeBinDir, "fake-tunnel");
	fs.writeFileSync(
		target,
		`#!/bin/sh\n` +
			`: > "$PROTO_FAKE_TUNNEL_ARGS"\n` +
			`for arg do printf '%s\\n' "$arg" >> "$PROTO_FAKE_TUNNEL_ARGS"; done\n` +
			`printf 'run\\n' >> "$PROTO_FAKE_TUNNEL_RUNS"\n` +
			`trap 'printf "SIGINT\\n" >> "$PROTO_FAKE_TUNNEL_SIGNALS"; exit 0' INT\n` +
			`trap 'printf "SIGTERM\\n" >> "$PROTO_FAKE_TUNNEL_SIGNALS"; exit 0' TERM\n` +
			`if [ -n "$PROTO_FAKE_TUNNEL_OUTPUT" ]; then printf '%s\\n' "$PROTO_FAKE_TUNNEL_OUTPUT"; fi\n` +
			`if [ -n "$PROTO_FAKE_TUNNEL_RESTART_MARKER" ]; then\n` +
			`  if [ ! -e "$PROTO_FAKE_TUNNEL_RESTART_MARKER" ]; then\n` +
			`    printf 'first\\n' > "$PROTO_FAKE_TUNNEL_RESTART_MARKER"\n` +
			`    exit 23\n` +
			`  fi\n` +
			`  printf 'restarted\\n' >> "$PROTO_FAKE_TUNNEL_RESTART_MARKER"\n` +
			`fi\n` +
			`if [ -n "$PROTO_FAKE_TUNNEL_EXIT_CODE" ]; then exit "$PROTO_FAKE_TUNNEL_EXIT_CODE"; fi\n` +
...
afterAll(async () => {
	...
	delete process.env.PROTO_FAKE_TUNNEL_ARGS;
	delete process.env.PROTO_FAKE_TUNNEL_RUNS;
	delete process.env.PROTO_FAKE_TUNNEL_SIGNALS;
	delete process.env.PROTO_FAKE_TUNNEL_OUTPUT;
	delete process.env.PROTO_FAKE_TUNNEL_EXIT_CODE;
	delete process.env.PROTO_FAKE_TUNNEL_RESTART_MARKER;
...
describe("startExposure tunnel adapters", () => {
	...
	it("reports absent adapter binaries without invoking the network", async () => {
		const emptyPath = fs.mkdtempSync(path.join(os.tmpdir(), "proto-no-tunnel-bin-"));
