/**
 * Fleet Enter contract: activating a non-remote agent row delegates to the
 * `focusAgent` dep (session focus proxy) and closes the fleet on success; a
 * focus failure keeps the fleet open and surfaces the error as a notice.
 */
...
import { AgentFleetOverlayComponent } from "@oh-my-pi/pi-coding-agent/modes/components/agent-fleet";
...
const TEST_CWD = path.resolve("agent-fleet-cwd");
...
function makeHub(focusAgent: (id: string) => Promise<void>) {
	...
	const hub = new AgentFleetOverlayComponent({
...
describe("Agent fleet Enter activation", () => {
	beforeAll(() => {
		initTheme();
	});
	...
	it("Enter focuses the selected agent and closes the fleet", async () => {
	...
	it("a focus failure keeps the fleet open and shows the error as a notice", async () => {
	...
	it("selector controller restores focus to the editor after Enter focuses an agent", async () => {
		...
		const editor = {};
		let capturedHub: AgentFleetOverlayComponent | undefined;
		const focusedIds: string[] = [];
		...
		const ctx = {
			keybindings: { getKeys: () => [] },
			ui: {
				showOverlay: (component: AgentFleetOverlayComponent) => {
		...
		controller.showAgentFleet(new SessionObserverRegistry());
...
describe("Agent fleet double-← gating", () => {
	beforeAll(() => {
		initTheme();
	});
	...
	function setup(_agents: AgentRegistry, sessionFile: string | null = null) {
		let shown: AgentFleetOverlayComponent | undefined;
		let overlayOptions: Record<string, unknown> | undefined;
		const shownReady = Promise.withResolvers<AgentFleetOverlayComponent>();
		const editor = {};
		const focusTargets: unknown[] = [];
		const ctx = {
			keybindings: { getKeys: () => [] },
			ui: {
				showOverlay: (component: AgentFleetOverlayComponent, options: Record<string, unknown>) => {
					shown = component;
	...
	it("requireContent keeps the fleet closed when only Main is registered", () => {
		const agents = AgentRegistry.global();
		agents.register({
			id: "Main",
			displayName: "Main",
		...
		controller.showAgentFleet(new SessionObserverRegistry(), { requireContent: true });

		expect(shown()).toBeUndefined();
	});

	it("requireContent opens the fleet once a subagent exists", () => {
		const agents = AgentRegistry.global();
	...
	it("armCloseTap lets a single ← dismiss the fleet the opening ←← raised", () => {
		const agents = AgentRegistry.global();
		// A parked/persisted agent opens the fleet under requireContent (issue #4780).
		agents.register({
			id: "Parked",
			displayName: "Parked",
			kind: "sub",
		...
		controller.showAgentFleet(new SessionObserverRegistry(), { requireContent: true, armCloseTap: true });

		const hub = shown();
		expect(hub).toBeDefined();
		expect(focusTargets.at(-1)).toBe(hub);

		// One ← — the editor's detector consumed the ←← that opened the fleet — now
		// closes it, returning focus to the editor. Without armCloseTap this ← only
		// primes the fleet's fresh detector and the user stays trapped.
...
describe("Agent fleet data refresh coalescing", () => {
	beforeAll(() => {
		initTheme();
	});
