...
test("user ~/.proto/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".proto", "agent", "RULES.md"),
...
test("project .proto/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".proto", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".proto", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".proto", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".proto", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".proto", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".proto", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
