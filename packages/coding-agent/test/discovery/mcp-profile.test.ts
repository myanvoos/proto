 ...
 *
 * A named profile relocates the agent directory to ~/.proto/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.proto/agent/mcp.json) via `ctx.home`, so a profile never saw its own
 ...
 *
 * `os.homedir()` is mocked so the *old* code path (ctx.home + ".proto/agent")
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.proto/profiles/<name>/agent).
