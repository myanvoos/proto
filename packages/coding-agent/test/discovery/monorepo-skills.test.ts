...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .proto/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".proto", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".proto", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".proto", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".proto", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".proto", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".proto", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".proto", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".proto", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".proto", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".proto", "skills"), "root-skill", "At repo root");
