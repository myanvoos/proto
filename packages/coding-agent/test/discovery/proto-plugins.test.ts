...
function buildExtensionPackage(packageDir: string, skillName = "my-skill"): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), proto: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearOmpExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "proto-plugins-"));
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".proto", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".proto", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("explicit-only CLI roots replace stale state and exclude every ambient package source", async () => {
	...
	const installed = path.join(home, ".proto", "plugins", "node_modules", "installed-extension");
	...
	writeFile(path.join(project, ".proto", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	writeFile(path.join(home, ".proto", "agent", "settings.json"), JSON.stringify({ extensions: [userExt] }));
	writeFile(
		path.join(home, ".proto", "plugins", "package.json"),
		JSON.stringify({ name: "proto-plugins", dependencies: { "installed-extension": "1.0.0" } }),
	);
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".proto", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".proto", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test(".mcp.json expands environment placeholders recursively", async () => {
	const variables = {
		PROTO_PLUGIN_COMMAND: "expanded-command",
		PROTO_PLUGIN_ARG: "expanded-arg",
		PROTO_PLUGIN_ENV: "expanded-env",
		PROTO_PLUGIN_CWD: path.join(tempDir, "expanded-cwd"),
		PROTO_PLUGIN_URL: "https://mcp.example.test",
		PROTO_PLUGIN_HEADER: "expanded-header",
		PROTO_PLUGIN_CLIENT_ID: "expanded-client-id",
	};
	...
	try {
		writeFile(
			...
			JSON.stringify({
				mcpServers: {
					stdio: {
						command: placeholder("PROTO_PLUGIN_COMMAND"),
						args: [placeholder("PROTO_PLUGIN_ARG")],
						env: { TOKEN: placeholder("PROTO_PLUGIN_ENV") },
						cwd: placeholder("PROTO_PLUGIN_CWD"),
					},
					http: {
						type: "http",
						url: placeholder("PROTO_PLUGIN_URL"),
						headers: { Authorization: `Bearer ${placeholder("PROTO_PLUGIN_HEADER")}` },
						oauth: { clientId: placeholder("PROTO_PLUGIN_CLIENT_ID") },
					},
				},
			}),
		);
		...
		expect(stdio).toMatchObject({
			command: variables.PROTO_PLUGIN_COMMAND,
			args: [variables.PROTO_PLUGIN_ARG],
			env: { TOKEN: variables.PROTO_PLUGIN_ENV },
			cwd: variables.PROTO_PLUGIN_CWD,
		});
		expect(http).toMatchObject({
			url: variables.PROTO_PLUGIN_URL,
			headers: { Authorization: `Bearer ${variables.PROTO_PLUGIN_HEADER}` },
			oauth: { clientId: variables.PROTO_PLUGIN_CLIENT_ID },
		});
	} finally {
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `proto plugin link`/`install`)", async () => {
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".proto", "plugins");
	const installed = path.join(pluginsDir, "node_modules", "my-project-ext");
	fs.mkdirSync(installed, { recursive: true });
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "proto-plugins.lock.json"),
		JSON.stringify({
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "proto-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "proto-plugins.lock.json"),
