...
async function claimTab(
	...
	bridge.cdpMessage(connId, JSON.stringify({ id: ++msgSeq, sessionId, method: "PROTO.claimTarget" }));
	await flush();
}

describe("RelayBridge tab grouping", () => {
	it("groups nothing on hello or tab lifecycle events — only claimed tabs join the proto group", () => {
		const bridge = new RelayBridge({ group: { title: "proto", color: "cyan" } });
	...
	it("groups exactly the tab a client claims", async () => {
		...
		expect(groups[0]!.title).toBe("proto");
	...
	it("never re-groups a tab the user pulled out of the proto group", async () => {
		const bridge = new RelayBridge({ group: { title: "proto", color: "cyan" } });
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
