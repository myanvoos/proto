/**
 * Unified `fleet` wait: one blocking primitive racing background jobs against
...
import { type CoordinationDetails, FleetTool } from "@oh-my-pi/pi-coding-agent/tools/fleet";
...
function makeSession(manager: AsyncJobManager | undefined): ToolSession {
	...
	// Structurally-partial test session: FleetTool only touches the fields above.
...
describe("fleet unified wait", () => {
	...
	test("an incoming message settles the wait while watched jobs keep running", async () => {
		...
		const tool = new FleetTool(makeSession(manager));
	...
	test("a settling job returns the snapshot exactly like the job poll", async () => {
		const registry = AgentRegistry.global();
		registry.register({ id: SELF_ID, displayName: "main", kind: "main", session: null });
		registry.register({ id: "Peer", displayName: "task", kind: "sub", parentId: SELF_ID, session: null });
	...
	test("bare wait ignores a detached ref whose running status is stale", async () => {
		...
		const result = await new FleetTool(makeSession(manager)).execute("call_4", { op: "wait", timeoutMs: 0 });
		const text = result.content[0]?.type === "text" ? result.content[0].text : "";
	...
	test("bare wait returns a message already queued on the bus", async () => {
		...
		const manager = new AsyncJobManager({ onJobComplete: () => {} });
		const result = await new FleetTool(makeSession(manager)).execute("call_5", { op: "wait" });
		const details = result.details as CoordinationDetails;
