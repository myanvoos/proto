...
import { fleetToolRenderer, type LaunchToolDetails } from "@oh-my-pi/pi-coding-agent/tools/fleet";
...
describe("fleet launch rendering", () => {
	it("is registered with merged call/result so the pending header is replaced, not stacked", () => {
		expect(Object.is(toolRenderers.fleet.renderResult, fleetToolRenderer.renderResult)).toBe(true);
		expect(toolRenderers.fleet.mergeCallAndResult).toBe(true);
	});
