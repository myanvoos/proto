...
describe("worker agent capability descriptions", () => {
	it("classifies bundled scout as the only read-only delegated agent", () => {
		...
		for (const name of ["worker", "lightbot", "reviewer", "designer"]) {
	...
	it("ships every bundled agent without prewalk; hand-off is opt-in via orchestrator.agentPrewalk", () => {
		const agents = loadBundledAgents();

		for (const name of ["worker", "scout", "lightbot", "reviewer", "designer", "librarian"]) {
