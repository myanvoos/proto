...
describe("extension load startup notifications", () => {
	it("formats load failures as sanitized single-line warnings for TUI and print startup paths", () => {
		...
		const extensionPath = path.join(homeDir, "proto-notification-fixture", "plugin\tname", "extension.ts");
		...
		expect(message?.startsWith("Failed to load extension ~/proto-notification-fixture/plugin")).toBe(true);
