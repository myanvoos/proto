...
describe("discoverAdvisorConfigs", () => {
	...
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "proto-advisor-config-"));
		await fsp.mkdir(path.join(tmp, ".git"));
		// Empty agent dir so the user-level search path can't pick up a real ~/.proto/WATCHDOG.yml.
		agentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "proto-advisor-agentdir-"));
	});
...
describe("WATCHDOG.yml file round-trip", () => {
	let tmp: string;
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "proto-advisor-file-"));
		await fsp.mkdir(path.join(tmp, ".git"));
	});
	afterEach(async () => {
		await fsp.rm(tmp, { recursive: true, force: true });
	...
	it("resolves project and user scope paths", () => {
		expect(advisorConfigFilePath("project", { projectDir: "/repo", agentDir: "/home/.proto" })).toBe(
			path.join("/repo", "WATCHDOG.yml"),
		);
		expect(advisorConfigFilePath("user", { projectDir: "/repo", agentDir: "/home/.proto" })).toBe(
			path.join("/home/.proto", "WATCHDOG.yml"),
		);
	});
});
...
describe("resolveAdvisorConfigEditPath", () => {
	let tmp: string;
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "proto-advisor-resolve-"));
	});
	afterEach(async () => {
		await fsp.rm(tmp, { recursive: true, force: true });
	});
...
describe("per-advisor enabled field", () => {
	it("preserves explicit true, explicit false, and absence through save and discovery", async () => {
		const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "proto-advisor-enabled-"));
		await fsp.mkdir(path.join(tmp, ".git"));
		try {
			const doc: WatchdogConfigDoc = {
				advisors: [
