...
function buildAmount(info: AntigravityQuotaInfo): UsageAmount {
	// Exhausted legacy counters omit remainingFraction and keep only resetTime: treat as blocked until reset.
	return buildPercentAmount(clampFraction(info.remainingFraction) ?? (info.resetTime ? 0 : undefined));
}
...
function getQuotaSummaryCounterKeys(
	...
	if (
		...
		// One shared third-party quota, duplicated into both model-family counters so Claude and GPT
		// requests each rank against it.
...
function buildQuotaSummaryReport(
	...
	const addBucket = (bucket: AntigravityQuotaSummaryBucket, group?: AntigravityQuotaSummaryGroup) => {
		...
		const window = parseWindow(
			{ resetTime: bucket.resetTime, windowId: bucket.window, windowLabel: descriptor?.label },
			descriptor,
		);
		...
		for (const counterKey of counterKeys) {
			const label =
				sharedGroup !== undefined
					...
					: counterKey === "google"
						? "Gemini"
						: (group?.displayName ?? bucket.displayName ?? "Usage");
			limits.push({
...
async function fetchAntigravityUsage(params: UsageFetchParams, ctx: UsageFetchContext): Promise<UsageReport | null> {
	...
	// The quota summary (what Antigravity's own quota UI reads) reports the rolling 5h and weekly
	// buckets even when neither is exhausted; older deployments fall back to the model catalog.
	...
	// Autocomplete models (`tab_*`, internal `chat_*`) report a bare `{ remainingFraction: 1 }` without a reset
	// on the Google counter; once the metered window is exhausted it would surface as a phantom "Daily 0%".
	// A counter with any windowed entry keeps only its windowed entries.
...
export function getAntigravityCounterKeyForModel(rawModelId: string | undefined): string | undefined {
	const modelId = rawModelId?.toLowerCase();
	if (!modelId) return undefined;
	if (modelId.startsWith("claude-")) return "anthropic";
	if (modelId.startsWith("gemini-") || modelId.startsWith("gemma-") || modelId.startsWith("tab_")) return "google";
...
export const antigravityRankingStrategy: CredentialRankingStrategy = {
	...
	// A 429 can carry a retry-after at the weekly reset while Google restores quota earlier; each backend
	// counter a fresh report covers can lift its own block.
