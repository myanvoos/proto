...
// Seat management lives on the Cascade backend, never on the `api.devin.ai` host stored as the
// credential endpoint (that one only mints the CLI session token).
...
interface DevinCreditBucket {
	...
	limit: number;
	used: number;
	// Server-reported balance; top-ups make it diverge from `limit - used`.
	available: number;
}
...
// Credit-billed plans leave the quota percents at their proto defaults, which would read as a fully
// consumed window; only surface a percent window the server dated or on an explicitly quota-billed plan.
...
function buildDevinReport(response: GetUserStatusResponse): UsageReport | null {
	...
	// The top-level `plan_info` is authoritative; the copy nested under `plan_status` only fills in
	// for older servers.
