/**
 * proto auth-gateway HTTP server.
 *
 ...
 * pure protocol translation: foreign wire → proto Context → pi-ai stream() →
 * proto events → foreign wire.
 *
 * Endpoints:
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (proto re-stamps every parsed Message). role +
 ...
 * (proto's own proxy discovery, Zed's openai_compatible provider, ...) read to
