...
export const cloudflareAiGatewayProvider = {
	...
	// Catalog rows ride the Anthropic route; each upstream namespace has its own gateway
	// endpoint. A caller-supplied concrete baseUrl (no placeholders) is kept as-is.
	prepareModel: model => {
		...
		if (model.id.startsWith("anthropic/")) {
			...
			return { ...model, api: "anthropic-messages", baseUrl, requestModelId };
		}
		const isOpenAIRoute = model.id.startsWith("openai/");
		if (isOpenAIRoute || model.id.startsWith("workers-ai/")) {
			// Workers AI keeps its namespaced id on the compat endpoint; OpenAI strips it.
			const requestModelId = isOpenAIRoute ? model.id.slice("openai/".length) : model.requestModelId;
	...
	prepareRequest: (model, options) => {
		...
		// Anthropic accepts the gateway token as x-api-key; OpenAI-format routes must carry it
		// only as cf-aig-authorization so no upstream bearer leaks through the gateway.
