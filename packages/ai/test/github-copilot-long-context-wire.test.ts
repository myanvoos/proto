 ...
 * hint (an oh-my-pi extension that preserves native-resolution images) with
 * an HTTP 400. The catalog resolves `supportsImageDetailOriginal` to `false`
 * for Copilot, and the Responses request builder degrades the hint to `auto`
 * so the wire stays valid. Every other host preserves `"original"`.
 */
