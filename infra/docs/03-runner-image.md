...
Build context lives at `/root/proto-kata-runner-image/`. The Dockerfile below is
...
> the `/root/proto-kata-runner-image/` copy is overwritten from it by the
...
```bash
#!/usr/bin/env bash
# Rebuild the preloaded proto-kata runner image, import it into k3s containerd,
...
#
#   ./reload.sh              # build tag proto-kata-runner:YYYY-MM-DD-HHMMSS
#   ./reload.sh 2026-06-20   # build tag proto-kata-runner:2026-06-20
...
case "$arg" in *:*) IMAGE="$arg";; *) IMAGE="proto-kata-runner:$arg";; esac

echo "==> [1/5] building $IMAGE"
DOCKER_BUILDKIT=1 docker build -t "$IMAGE" -t proto-kata-runner:preloaded .
...
sed -i "s#image: proto-kata-runner:.*#image: $IMAGE#" /root/arc-proto-values.yaml
helm upgrade proto-kata --namespace arc-runners --version 0.14.2 \
  -f /root/arc-proto-values.yaml \
  oci://ghcr.io/actions/actions-runner-controller-charts/gha-runner-scale-set >/dev/null

echo "==> [5/5] verifying rollout"
live="$(kubectl get autoscalingrunnerset proto-kata -n arc-runners -o jsonpath='{.spec.template.spec.containers[0].image}')"
...
```bash
cd /root/proto-kata-runner-image
./reload.sh
```
...
`$1`: no arg gives a timestamped `proto-kata-runner:YYYY-MM-DD-HHMMSS`; an argument
containing a colon (`foo:bar`) is used as an explicit `repo:tag`; anything else
is treated as a tag suffix on `proto-kata-runner:`.

**[1/5] build.** `DOCKER_BUILDKIT=1 docker build` tags the result twice: the
immutable `$IMAGE` (dated) and the moving `proto-kata-runner:preloaded` alias.
...
`image: proto-kata-runner:...` line in `/root/arc-proto-values.yaml` to the new tag,
...
- `docker save | k3s ctr -n k8s.io images import` places the image directly into
  ...
  reference `proto-kata-runner:<tag>` to `docker.io/library/proto-kata-runner:<tag>`
...
| --- | --- | --- | --- | --- |
| `proto-kata-runner:YYYY-MM-DD-HHMMSS` | immutable | yes | yes | the build of record; what runners actually boot |
| `proto-kata-runner:preloaded` | moving | no | no | docker-local alias to the most recent build |
...
- Live example at time of writing: the scale set references
  `proto-kata-runner:2026-06-15-002621`, held in containerd as
  `docker.io/library/proto-kata-runner:2026-06-15-002621`.
...
```bash
docker run --rm --entrypoint bash proto-kata-runner:preloaded -lc '
  set -e
  for b in gh fd rg magick bun cargo rustc pkg-config clang lld zstd cmake ninja cargo-nextest musl-gcc aarch64-linux-gnu-gcc; do
    command -v "$b" >/dev/null || { echo "MISSING: $b"; exit 1; }
  done
...
```bash
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
kubectl get autoscalingrunnerset proto-kata -n arc-runners \
  -o jsonpath='{.spec.template.spec.containers[0].image}{"\n"}'
k3s ctr -n k8s.io images ls | grep proto-kata-runner
```
...
kubectl run preload-verify -n arc-runners --restart=Never \
  --image=proto-kata-runner:2026-06-15-002621 \
