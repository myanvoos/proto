...
use clap::{Arg, ArgMatches, Command as ClapCommand, builder::ValueParser};
use serde_json::{Value, json};

use crate::host::{Host, Utility, matches_parser, util};

const ADDR_VAR: &str = "PI_PYSH_ADDR";
const TOKEN_VAR: &str = "PI_PYSH_TOKEN";
...
impl Utility for Python {
	...
	fn run(self, host: &mut Host) -> i32 {
		let argv: Vec<OsString> = self
			...
			.cloned()
			.collect();
		match plan(&argv, host) {
			Plan::Cell { code, stdin_body } => match run_kernel_cell(host, &code) {
...
fn plan(argv: &[OsString], host: &mut Host) -> Plan {
	let mut index = 0;
...
fn run_kernel_cell(host: &mut Host, code: &str) -> CellOutcome {
