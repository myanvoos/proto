...
mod tests {
	...
	#[test]
	fn host_proto_num_threads_forces_count() {
		let (code, capture) = run_in(&[("PROTO_NUM_THREADS", "3")], &[]);
		assert_eq!((code, capture.out(), capture.err()), (0, "3\n".to_string(), String::new()));
	}

	#[test]
	fn proto_thread_limit_caps_proto_num_threads() {
		let (code, capture) = run_in(
			&[("PROTO_NUM_THREADS", "64"), ("PROTO_THREAD_LIMIT", "2")],
			&[],
		);
	...
	#[test]
	fn all_prints_positive_integer_and_ignores_proto_num_threads() {
		let (code, capture) = run_in(&[("PROTO_NUM_THREADS", "0")], &["--all"]);
		assert_eq!(code, 0);
	...
	fn process_environment_is_not_consulted() {
		...
		unsafe { std::env::set_var("PROTO_NUM_THREADS", "1234") };
		let (code, capture) = run_in(&[], &[]);
		unsafe { std::env::remove_var("PROTO_NUM_THREADS") };
	...
	fn ignore_subtracts_and_floors_at_one() {
		let (code, capture) = run_in(&[("PROTO_NUM_THREADS", "8")], &["--ignore=3"]);
		assert_eq!((code, capture.out()), (0, "5\n".to_string()));

		let (code, capture) = run_in(&[("PROTO_NUM_THREADS", "2")], &["--ignore=5"]);
