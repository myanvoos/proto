...
const SUMS = {
	"proto-darwin-arm64": "darwin_arm64_sha",
	"proto-darwin-x64": "darwin_x64_sha",
	"proto-linux-arm64": "linux_arm64_sha",
	"proto-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["proto-*"].first` returns `nil`, and
	// `bin.install nil => "proto"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["proto-darwin-arm64", "proto-darwin-x64", "proto-linux-arm64", "proto-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.proto` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"proto", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
