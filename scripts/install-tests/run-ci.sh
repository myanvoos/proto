...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/proto "$BINARY_DIR/proto"
smoke_cli "$BINARY_DIR/proto"
...
(
   ...
   smoke_cli "$BUN_INSTALL/bin/proto"
)
...
for pkg in utils wire omptype hashline catalog ai mnemopi agent tui stats collab-web; do
   (
...
#    `bin.proto` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$omptype_tgz" "$natives_tgz" "$hashline_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$coding_agent_tgz" "$collab_web_tgz"
   ...
   smoke_cli ./node_modules/.bin/proto
)
