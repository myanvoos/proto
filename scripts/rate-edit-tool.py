...
def resolve_proto_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `proto` on PATH. Set --proto-bin or PROTO_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through proto RPC mode."
    )
    parser.add_argument("--proto-bin", default=os.environ.get("PROTO_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"proto-fixture-runs-{timestamp}"
    )
