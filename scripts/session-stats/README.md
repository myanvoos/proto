...
scripts/session-stats/
  sync.py          # walks ~/.proto/agent/sessions/ and populates ss_* tables
...
credentials resolved through proto's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.proto/stats-audit-cache.json` (keyed by session id +
