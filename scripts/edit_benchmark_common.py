...
def resolve_repo_proto_bin() -> str | None:
...
def resolve_proto_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_proto_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("proto")
    if not found:
        raise SystemExit(
            "Could not find `proto` on PATH and could not resolve the repo CLI. Set --proto-bin or PROTO_BIN."
        )
    return found
...
async def run_all(
    ...
    proto_bin = resolve_proto_bin(args.proto_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--proto-bin",
        default=os.environ.get("PROTO_BIN"),
        help="Executable to launch. Defaults to the repo checkout CLI, then falls back to `proto` on PATH.",
    )
    parser.add_argument(
