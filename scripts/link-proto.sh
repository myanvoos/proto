...
# `packages/coding-agent/scripts/proto`. See that wrapper's header comment for the
...
target=$repo_root/packages/coding-agent/scripts/proto

if [ ! -x "$target" ]; then
	echo "link-proto: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/proto"
echo "link-proto: linked $global_bin/proto -> $target"
